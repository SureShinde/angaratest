...
class SFC_Kount_Model_Observer
{
    ...
    public function onSalesOrderPaymentPlaceStart($observer)
    {
        ...
        if ($payment->getMethodInstance()->getCode() == 'paypal_standard' || $payment->getMethodInstance()->getCode() == 'paypal_express')
            return;
    ...
    public function onCheckoutSuccessAction($observer)
    {
        ...
        foreach($orderIds as $orderId) {
            ...
            if($order->getData('kount_ris_response') == SFC_Kount_Helper_Ris::RESPONSE_DECLINE) {
                if($order->getPayment()->getMethod() == 'paypal_direct'){
                    Mage::log('Pausing for Paypal Direct', Zend_Log::INFO, SFC_Kount_Helper_Data::KOUNT_LOG_FILE);
                    sleep(20);
                }
    ...
    public function onSalesOrderServiceQuoteSubmitFailure($observer)
    {
        ...
        // Skip paypal standard
        if ($order->getPayment()->getMethodInstance()->getCode() == 'paypal_standard' || $order->getPayment()->getMethodInstance()->getCode() == 'paypal_express')
            return;
