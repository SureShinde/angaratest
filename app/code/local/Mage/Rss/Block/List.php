...
*/
    ...
    */
	...
	//	Function added by Vaseem to get top categories for rss
	public function getTopCategoriesIds()
    {
        return Mage::getStoreConfig('rss/catalog/top_category_ids');
    }
    ...
    public function CategoriesRssFeed()
    {
        ...
        if((bool)Mage::getStoreConfig($path)){
			...
			//	Remove category from array modified by Vaseem
			$removeCatIdArray	=	array('9','247','248','282','295','268','227','286','358','246','348','406','414','415','417','441','325');
            foreach ($nodes as $node) {
				if(!in_array($node->getId(),$removeCatIdArray)){
	                $nodeIds[] = $node->getId();
				}
            }
			//	Add few more category to rss feeds modified by Vaseem
			$addCatIdArray	=	array('421','371','328','77');
			$nodeIds		=	array_unique(array_merge($nodeIds,$addCatIdArray));	
