...
class Mage_Adminhtml_Block_Sales_Order_View_Info extends Mage_Adminhtml_Block_Sales_Order_Abstract
{
	...
	/**
     * Make the order ids link to order
     * @return link
     */
	public function getOrderLink($orderIncrementId)
    {
		if (empty($orderIncrementId)) {
            return ;
        }
		$write 		= 	Mage::getSingleton('core/resource')->getConnection('core_read');
		$result		=	$write->query("SELECT entity_id FROM `sales_flat_order` WHERE `increment_id` =".(int)$orderIncrementId);
		$row 		= 	$result->fetch();
		$orderID	=	$row['entity_id'];
		return $this->helper('adminhtml')->getUrl('*/sales_order/view', array('order_id' => $orderID));
    }
}
