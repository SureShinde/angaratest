...
class Mage_GoogleCheckout_Model_Api_Xml_Checkout extends Mage_GoogleCheckout_Model_Api_Xml_Abstract
{
	...
	protected $_total = 0;
    protected $_min4freeShip = 150;
    protected $_freeShipMethod = "freeshipping";
...
EOT;
        ...
        foreach ($this->getQuote()->getAllItems() as $item) {
			...
			// Angara Modification Start
			$this->_total += $item->getBaseCalculationPrice()*$item->getQty();
			// Angara Modification End
            ...
            <item>
				...
				<item-description><![CDATA[{$item->getSku()}]]><![CDATA[{$item->getDescription()}]]></item-description>
...
EOT;*/
		$xml = <<<EOT
            <merchant-calculations>
                <merchant-calculations-url><![CDATA[http://www.angara.com/googlecheckout/api]]></merchant-calculations-url>
            </merchant-calculations>
EOT;
		// Angara Modification End
        return $xml;
        ...
        foreach ($methods['method'] as $i => $method) {
            ...
            if ($carrierCode) {
                ...
                if ($carrier) {
					...
					// Angara Modification Start
					if($methodCode==$this->_freeShipMethod){
						if($this->_total>$this->_min4freeShip){
							$defaultPrice=0; 
						}else{ 
...
EOT;
    ...
    protected function _getAllTaxTablesXml()
    {
        ...
        if ($isDefaultTaxTablesDisabled) {
			...
			$tax_state_wise_xml = '<tax-tables merchant-calculated="true">
			  <default-tax-table>
				<tax-rules>	
				  <default-tax-rule>
					<shipping-taxed>true</shipping-taxed>
