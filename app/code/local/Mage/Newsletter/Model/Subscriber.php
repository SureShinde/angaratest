...
class Mage_Newsletter_Model_Subscriber extends Mage_Core_Model_Abstract
{
	...
	const XML_PATH_SUCCESS_EMAIL_TEMPLATE_CART  = 'newsletter/subscription/success_email_template_cart';
	const XML_PATH_SUCCESS_EMAIL_TEMPLATE_CATALOG  = 'newsletter/subscription/success_email_template_catalog';
	const XML_PATH_SUCCESS_EMAIL_TEMPLATE_CATALOG_BOTTOM  = 'newsletter/subscription/success_email_template_catalog_bottom';
	const XML_PATH_SUCCESS_EMAIL_TEMPLATE_HOME  = 'newsletter/subscription/success_email_template_home';
	const XML_PATH_SUCCESS_EMAIL_TEMPLATE_FOOTER  = 'newsletter/subscription/success_email_template_footer';
	const XML_PATH_SUCCESS_EMAIL_TEMPLATE_MILITARY  = 'newsletter/subscription/success_email_template_military';
	const XML_PATH_SUCCESS_EMAIL_TEMPLATE_POPUP  = 'newsletter/subscription/success_email_template_popup';
     ...
     * @return int
     */
    public function subscribe($email,$location=null)
    {
        ...
        try {
			...
			$storeid_hp= $this->getStoreId(); 
			$custid_hp = $this->getCustomerId(); 
			$subsemal_hp = $this->getSubscriberEmail(); 
			$substatus_hp=$this->getStatus(); 
			$confirm_hp= $this->getSubscriberConfirmCode();
            
            $loc=isset($_POST['newslettertype'])?$_POST['newslettertype']:(!is_null($location)?$location:'');
            
			$query =  "insert into newsletter_subscriber set store_id='$storeid_hp',customer_id='$custid_hp', subscriber_email='$subsemal_hp', subscriber_status='$substatus_hp', subscriber_confirm_code='$confirm_hp', ref_url='" .$_POST['return_url'] ."', `sub_type`='" .  $loc  . "',`capture_date`='" . date('Y-m-d') . "', `capture_time`='" . date('H:i:s') . "', user_ip='" . $this->getRealIpAddr() . "'";
			$write->query($query);
			
			$cust = Mage::getModel('newsletter/subscriber')->setWebsiteId(Mage::app()->getStore()->getWebsiteId());
			$cust = $cust->loadByEmail($subsemal_hp);
			
			Mage::dispatchEvent('newsletter_subscriber_event', array('email' => $subsemal_hp ,'id' => $cust->getId()));
			// Angara Modification End
			
			if($location != 'OrderEmail'){
				if ($isConfirmNeed === true
					&& $isOwnSubscribes === false
				) {
    ...
    public function sendConfirmationSuccessEmail()
    {
		...
		// Angara Modification Start
		if($_POST['newslettertype']=="03-Cart-Ruby-Pendant-Contest")
		{
			$email->sendTransactional(
				Mage::getStoreConfig(self::XML_PATH_SUCCESS_EMAIL_TEMPLATE_CART),
				Mage::getStoreConfig(self::XML_PATH_SUCCESS_EMAIL_IDENTITY),
				$this->getEmail(),
				$this->getName(),
				array('subscriber'=>$this)
		...
		else if($_POST['newslettertype']=="01-Home-Ruby-Pendant-Contest")
		{
			$email->sendTransactional(
				Mage::getStoreConfig(self::XML_PATH_SUCCESS_EMAIL_TEMPLATE_HOME),
				Mage::getStoreConfig(self::XML_PATH_SUCCESS_EMAIL_IDENTITY),
				$this->getEmail(),
				$this->getName(),
				array('subscriber'=>$this)
		...
		else if($_POST['newslettertype']=="03-Home-Instant-Offer")
		{
			$email->sendTransactional(
				Mage::getStoreConfig(self::XML_PATH_SUCCESS_EMAIL_TEMPLATE_HOME),
				Mage::getStoreConfig(self::XML_PATH_SUCCESS_EMAIL_IDENTITY),
		...
		else if($_POST['newslettertype']=="02-Catalog-Ruby-Pendant-Contest")
		{
			$email->sendTransactional(
				Mage::getStoreConfig(self::XML_PATH_SUCCESS_EMAIL_TEMPLATE_CATALOG),
				Mage::getStoreConfig(self::XML_PATH_SUCCESS_EMAIL_IDENTITY),
				$this->getEmail(),
				$this->getName(),
				array('subscriber'=>$this)
		...
		// start code by anil
		else if($_POST['newslettertype']=="99-Sapphire-Sweepstake")
		{
			$email->sendTransactional(
				Mage::getStoreConfig(self::XML_PATH_SUCCESS_EMAIL_TEMPLATE_CATALOG_BOTTOM),
				Mage::getStoreConfig(self::XML_PATH_SUCCESS_EMAIL_IDENTITY),
				$this->getEmail(),
				$this->getName(),
				array('subscriber'=>$this)
		...
		// end code by anil
		else if($_POST['newslettertype']=="10-Catalog-Instant-Offer")
		{
			$email->sendTransactional(
				Mage::getStoreConfig(self::XML_PATH_SUCCESS_EMAIL_TEMPLATE_CATALOG),
				Mage::getStoreConfig(self::XML_PATH_SUCCESS_EMAIL_IDENTITY),
		...
		else if(($_POST['newslettertype']=="07-Footer-Stripe"))
		{
			$email->sendTransactional(
				Mage::getStoreConfig(self::XML_PATH_SUCCESS_EMAIL_TEMPLATE_FOOTER),
				Mage::getStoreConfig(self::XML_PATH_SUCCESS_EMAIL_IDENTITY),
				$this->getEmail(),
				$this->getName(),
				array('subscriber'=>$this)
		...
		else if(($_POST['newslettertype']=="04-Home-Pop-Up-Sign-Up") || ($_POST['newslettertype']=="05-Catalog-Pop-Up-Sign-Up") || ($_POST['newslettertype']=="06-Other-Pop-Up-Sign-Up"))
		{
			$email->sendTransactional(
				Mage::getStoreConfig(self::XML_PATH_SUCCESS_EMAIL_TEMPLATE_POPUP),
				Mage::getStoreConfig(self::XML_PATH_SUCCESS_EMAIL_IDENTITY),
				$this->getEmail(),
				$this->getName(),
				array('subscriber'=>$this)
		...
		else if(($_POST['newslettertype']=="11-Military-Offer"))
		{
			$email->sendTransactional(
				Mage::getStoreConfig(self::XML_PATH_SUCCESS_EMAIL_TEMPLATE_MILITARY),
				Mage::getStoreConfig(self::XML_PATH_SUCCESS_EMAIL_IDENTITY),
				$this->getEmail(),
				$this->getName(),
				array('subscriber'=>$this)
		...
		//	S:VA	Check if email is already subscribed to other list(s)
		$mcapi			= 	new MCAPI('d493d52c7900896ab9d824f88de117fc-us8');	
		$customeremail 	= 	$this->getEmail();
		$checkStatus	= 	$mcapi->listsForEmail( $customeremail );
		
			// code for sending welcome email via Mandrill service :S
				$fname = $this->getName();
				if( empty( $checkStatus ) ){
					$mailchimpListId	=	Mage::getStoreConfig('angarainfo/mailchimp_settings/mailchimp_list_id');	//	S:VA
					$mcapi->listSubscribe($mailchimpListId, $customeremail, $merge_vars=NULL, $email_type='html', $double_optin=false, $update_existing=false,$replace_interests=false, $send_welcome=false);
				}
				$mandrillApiKey	=	Mage::getStoreConfig('angarainfo/mandrill_settings/mandrill_api_key');	//	S:VA
				$mandrill= new Mandrill($mandrillApiKey);	
				$message = array('to' => array(array('email' => $customeremail, 'name' => $fname)),
				...
				setcookie('popupvisitor', '2', time() + (30*24*60* 60), '/', '.angara.com');
				// code for sending welcome email via Mandrill service :E	
		}
		// Angara Modification End
