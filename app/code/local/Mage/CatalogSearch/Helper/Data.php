...
class Mage_CatalogSearch_Helper_Data extends Mage_Core_Helper_Abstract
{
        ...
        if (!isset($this->_queryText)) {
            ...
            if ($this->_queryText === null) {
                ...
                $this->_queryText = is_array($this->_queryText) ? ''
                    : preg_replace('/[^\w ]/','', $stringHelper->cleanString(trim($this->_queryText)));
    ...
    public function prepareIndexdata($index, $separator = ' ')
    {
		...
		if(!empty($_index))
        	return $separator.join($separator, $_index).$separator;
		else
			return join($separator, $_index);
    }
	
	public function getGrammerRules(){
		$_rules = Mage::getModel('digger/grammar')->getGrammarRules();
		return $_rules;
	}
	
	public function parseGrammerRule($rule, $data, $separator = ' '){
		preg_match_all('/{(.*?)}/', $rule, $matches);
		if(isset($matches[0])){
			$replaceArray = $matches[0];
			$attributes = $matches[1];
			if(count(array_intersect_key(array_flip($attributes), $data)) === count($attributes)) {
				$results = array($rule);
				foreach($replaceArray as $key => $replaceString){
					$values = explode($separator, $data[$attributes[$key]]);
