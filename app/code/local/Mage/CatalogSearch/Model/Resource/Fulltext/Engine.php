...
class Mage_CatalogSearch_Model_Resource_Fulltext_Engine extends Mage_Core_Model_Resource_Db_Abstract
{
    ...
    public function saveEntityIndexes($storeId, $entityIndexes, $entity = 'product')
    {
        ...
        foreach ($entityIndexes as $entityId => $index) {
            $data[] = array(
				...
				'search_weight' => $index['search_weight'],
        ...
        if ($data) {
            Mage::getResourceHelper('catalogsearch')
                ->insertOnDuplicate($this->getMainTable(), $data, array('data_index','updated_at'));
        }
	...
	/**
     * Update entity update_at in fulltext search table
     *
     * @param int $storeId
     * @param int $entityIds
     * @param string $entity 'product'|'cms'
     * @return Mage_CatalogSearch_Model_Resource_Fulltext_Engine
     */
    public function updateIndex($storeId = null, $entityIds = null, $entity = 'product')
    {
        $where = array();
        ...
        if (!is_null($entityIds)) {
            $where[] = $this->_getWriteAdapter()->quoteInto('product_id IN (?)', $entityIds);
        }
    ...
    {
		/* Writing own grammer rules : Hitesh Agarwal */
		$_index = array();
		$_rules = Mage::helper('catalogsearch')->getGrammerRules();
		foreach($_rules as $_rule){
			$parsedRulesIndex = Mage::helper('catalogsearch')->parseGrammerRule($_rule, $index, $separator);
			if($parsedRulesIndex){
				$_index[] = $parsedRulesIndex;
			}
		}
		if(isset($index['tags'])){
        ...
        return Mage::helper('catalogsearch')->prepareIndexdata($_index, $separator);
    }
