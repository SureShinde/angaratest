...
class Mage_Tax_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	public function getEasyPayOptions($product, $price)
    {		
        $easy_arr = array();
		
		if($price!='' && $price>0){
			$opt1 = $price;
			$opt2 = round($price/2,2);
			$easy_arr = array(
				'0'=>$opt1,
				'1'=>$opt2
			);
			$productCategories = $product->getCategoryIds();
			if(($price < 2500) && (!in_array('73', $productCategories) && !in_array('435', $productCategories))){
				array_push($easy_arr, round($price/3,2));
			}
		}
		return $easy_arr;				
    }
