...
class Mage_Sitemap_Model_Resource_Catalog_Category extends Mage_Sitemap_Model_Resource_Catalog_Abstract
{
    ...
    public function getCollection($storeId)
    {
        ...
        $this->_select = $this->_getWriteAdapter()->select()
            ...
            ->from(array('e' => $this->getMainTable()), array($this->getIdFieldName(), 'level'))		//	S:VA
            ->joinLeft(
        ...
        while ($row = $query->fetch()) {
			...
			$pos_new1 = stripos($row['url'], 'featured');
			$pos_new2 = stripos($row['url'], 'earrings/diamondstud');
			if ($pos_new1 !== false || $pos_new2 !== false) {
				continue;
			}	
    ...
    protected function _prepareCategory(array $categoryRow)
    {
		...
		$category->setLevel($categoryRow['level']);		//	S:VA
        return $category;
    }
