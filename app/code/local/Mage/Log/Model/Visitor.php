...
class Mage_Log_Model_Visitor extends Mage_Core_Model_Abstract
{
    ...
    public function initByRequest($observer)
    {
		...
		/* Code added for refresh the channel data on page load */ 
		/* Asheesh:Start */
		$channel = Mage::getSingleton('core/session')->getVisitorChannel();
		if($channel) {
			$channel = Mage::getModel('promotions/channel')->load($channel->getId());
			if($channel && $channel->getStatus()){
				Mage::getSingleton('core/session')->setVisitorChannel($channel);
			} else {
				$channel = Mage::getModel('promotions/channel')->load(1);
				Mage::getSingleton('core/session')->setVisitorChannel($channel);
			}
		}
		/* Asheesh:End */
    ...
    public function saveByRequest($observer)
    {
        ...
        try {
			Mage::log($_SERVER['REMOTE_ADDR'].' - Old Session Id: '.$this->getSessionId() .' New SessionId: '.$this->_session->getSessionId(),null,'session_id.log',true);
            $this->setLastVisitAt(now());
			$this->setSessionId($this->_session->getSessionId());
            $this->save();
