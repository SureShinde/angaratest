...
abstract class Mage_Paypal_Controller_Express_Abstract extends Mage_Core_Controller_Front_Action
{
    ...
    public function cancelAction()
    {
		...
		// check for shipping getting zero in cart when cancel order 
		Mage::getSingleton('core/session')->setCancelPaypalCheckout('1');
        $this->_redirect('checkout/cart');
    }
    ...
    public function reviewAction()
    {
        try {
			...
			$availableShippingMethod	=	Mage::helper('function')->getAvailableShippingMethods();
			//Mage::log(print_r($availableShippingMethod, true), null, 'paypal.log', true);
			$firstAvailableShipping		=	'freeshipping_freeshipping';
			if(count($availableShippingMethod) > 0){
				$tempShipping				=	current($availableShippingMethod);
				$firstAvailableShipping		=	$tempShipping['code'];
			}
			if($this->_getQuote()->getShippingAddress()->getData('country_id') == 'US'){
				$this->_checkout->updateShippingMethod($firstAvailableShipping);	//	S:VA
			}
			else{
				$this->_checkout->updateShippingMethod('angnonusflatrate_angnonusflatrate');
			}
