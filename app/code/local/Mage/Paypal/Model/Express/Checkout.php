...
class Mage_Paypal_Model_Express_Checkout
{
    ...
    public function start($returnUrl, $cancelUrl)
    {
		...
		//Added by Vaseem Starts
		$quote = $this->_quote;
		$shippingAddress = $quote->getShippingAddress();
		//echo '<pre>'; print_r($shippingAddress); die;
		/*echo '<br>city->'.$shippingAddress->getCity();
		echo '<br>getCountryId->'.$shippingAddress->getCountryId();
		echo '<br>getRegion->'.$shippingAddress->getRegion();
		echo '<br>getStreet->'.$shippingAddress->getStreet();
		echo '<pre>'; print_r($shippingAddress->getStreet());
		echo '<br>getPostcode->'.$shippingAddress->getPostcode();
		echo '<br>getRegionId->'.$shippingAddress->getRegionId();
		echo '<br>city->'.$shippingAddress->getCity();*/

		//	Check if user coming from checkout page with blank shipping details
		...
		//	Getting the customer paypal details from session
		$paypalSession 	= 	Mage::getSingleton('core/session')->getData('paypal_customer_details');
		//echo '<pre>'; print_r($paypalSession); die;		

		if($paypalSession['email']!=''){
			//	Setting the customer paypal values
			$this->_quote->getShippingAddress()
						->setCountryId($paypalSession['country_id'])
						->setCity($paypalSession['city'])
						->setPostcode($paypalSession['postcode'])
						->setRegionId($paypalSession['region_id'])
						->setRegion($paypalSession['region'])
						->setFirstname($paypalSession['firstname'])
						->setLastname($paypalSession['lastname'])
						->setRegionId($paypalSession['region_id'])
						->setStreet($paypalSession['street'])
						->setCompany($paypalSession['company'])
						;
			$this->_quote->getShippingAddress()->save();
			
			//	Settings Additional information to paypal
			if($paypalSession['telephone']!=''){
				$this->_quote->getShippingAddress()->setTelephone($paypalSession['telephone']);
				$this->_quote->getShippingAddress()->save();
			}			
		}
		//Added by Vaseem Ends
    ...
    public function returnFromPaypal($token)
    {
		...
		//	Added by Vaseem Starts
		//echo '<pre>'; print_r($exportedShippingAddress); die;		
		
		$session = Mage::getSingleton('core/session');
		$session->setData('paypal_customer_details', array('company' 	=> 	$exportedShippingAddress->getCompany(), 
															'email' 	=> 	$exportedShippingAddress->getEmail(),
															'firstname'	=>	$exportedShippingAddress->getFirstname(),
															'lastname'	=>	$exportedShippingAddress->getLastname(),
															'country_id'=>	$exportedShippingAddress->getCountryId(),
															'region'	=>	$exportedShippingAddress->getRegion(),
															'city'		=>	$exportedShippingAddress->getCity(),
															'street'	=>	$exportedShippingAddress->getStreet(),
															'postcode'	=>	$exportedShippingAddress->getPostcode(),
															'region_id'	=>	$exportedShippingAddress->getRegionId(),
															'prefix'	=>	$exportedShippingAddress->getPrefix(),
															'middlename'=>	$exportedShippingAddress->getMiddlename(),
															'suffix'	=>	$exportedShippingAddress->getSuffix(),
															'telephone'	=>	$exportedShippingAddress->getTelephone(),			//	Additional Information
															'customer_notes'=>	$exportedShippingAddress->getCustomerNotes()	//	Additional Information
														   )
						 );
		$paypalSession 	= 	Mage::getSingleton('core/session')->getData('paypal_customer_details');
		//echo '<pre>'; print_r($paypalSession); die;
