...
class Mage_Checkout_Block_Cart_Totals extends Mage_Checkout_Block_Cart_Abstract
{
	...
	/*
		S:VA	Adding subtotal + discount
	*/
	public function displayStrikeGrandtotal()
    {
		$pageType	=	Mage::helper('function')->getPageType();
		$isIPad		=	Mage::helper('function')->isIPad();
		if($pageType=='cart' && $isIPad){
			$quote  	= $this->getQuote();
			...
			//return Mage::app()->getStore()->getBaseCurrency()->format($total, array(), true);
			return '<del class="high-padding-right">'.Mage::app()->getLocale()->currency(Mage::app()->getStore()->getCurrentCurrencyCode())->getSymbol().$total.'</del>';
		}
    }
