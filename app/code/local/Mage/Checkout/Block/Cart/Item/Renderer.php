...
class Mage_Checkout_Block_Cart_Item_Renderer extends Mage_Core_Block_Template
{
    ...
    {
		$recInfo = $this->_item->getBuyRequest()->getdata();
		$result = array();
		
		if(isset($recInfo['easyopt'])){
			$tmp = explode('_',$recInfo['easyopt']);
			if(isset($tmp[0]) && $tmp[0]>0){
				$result['total_installment'] = $tmp[0]+1;
				$result['installment_price'] = $tmp[1];
				$result['qty'] = $this->_item->getQty();
				return $result;
			}
		}
        return false;
    ...
    public function getProductUrl()
    {
		...
		foreach($this->getOptionList() as $option){
			if(isset($option['code'])){
				/* $urlParams[] = $option['code'].'='.urlencode($option['value']); */ /* For seo friendly URL */
				$urlParams[] = $option['code'].'='.$option['value_id'];
			} elseif(isset($option['label']) && isset($option['value']) && $option['value']) {
				if($option['label'] == 'Engraving') {
					$engraving = explode('(',$option['value']);
					$engravingText = isset($engraving[0])?str_replace('(','',$engraving[0]):'';
					$engravingFont = isset($engraving[1])?str_replace(')','',$engraving[1]):'';
					
					$engravingText = str_replace('&amp;','&',$engravingText);	//	S:VA	Resolved issue with  David & Andrea 10/12/14
					$engravingText = trim($engravingText);
					$engravingFont = trim($engravingFont);
					if(strlen($engravingText) > 0) {
						$urlParams[] = 'engraving'.'='.urlencode($engravingText);
						$urlParams[] = 'engraving_font'.'='.urlencode($engravingFont);
					} else {
						$urlParams[] = preg_replace('/\s+/', '_', strtolower($option['label'])).'='.urlencode($option['value']);
					}
				} else {
					$urlParams[] = preg_replace('/\s+/', '_', strtolower($option['label'])).'='.urlencode($option['value']);
				}
		...
		if($_recInfo){
			$instalmentNo = $this->getItem()->getNoOfInstallment();
			if($instalmentNo > 1) {
				$urlParams[] = 'easyopt'.'='.$instalmentNo;
			}
		}
		/* Jewelry Appraisal and Warranty Plan */
		$info = $this->getItem()->getBuyRequest();
		if($info->getData('appraisal'))
			$urlParams[] = 'appraisal'.'='.$info->getData('appraisal');
		if($info->getData('insurance'))
			$urlParams[] = 'insurance'.'='.$info->getData('insurance');
		
		if(!empty($urlParams)){
			return $product->getUrlModel()->getUrl($product).'?'.implode('&',$urlParams);
		}
