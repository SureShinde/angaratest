...
class Mage_Checkout_Block_Onepage_Success extends Mage_Core_Block_Template
{
    ...
    protected function _prepareLastOrder()
    {
		...
		// base_discount_amount, base_grand_total, base_subtotal
		// Item id, product ID, sku, qty, amount, name
        ...
        if ($orderId) {
            ...
            if ($order->getId()) {
				...
				foreach ($order->getAllVisibleItems() as $item) {
					$items[$item->getId()]['sku'] = $item->getSku();
					$items[$item->getId()]['product_id'] = $item->getProductId();
					$items[$item->getId()]['name'] = $item->getName();
					$items[$item->getId()]['qty'] = $item->getQtyOrdered();
					$items[$item->getId()]['amount'] = $item->getPrice();
				}
