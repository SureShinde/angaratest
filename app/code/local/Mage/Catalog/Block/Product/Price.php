...
class Mage_Catalog_Block_Product_Price extends Mage_Catalog_Block_Product_Abstract
{
	...
	public function getDotdDiscount(){
		$oCoupon = Mage::getModel('salesrule/coupon')->load('DOTD25', 'code');
		$oRule = Mage::getModel('salesrule/rule')->load($oCoupon->getRuleId());
		$data = unserialize($oRule->getActionsSerialized());
		if(strtoupper($data['conditions'][0]['value']) == $this->getProduct()->getSku()){
			return $oRule->getDiscountAmount();
		}
		return 0;
	}
}
