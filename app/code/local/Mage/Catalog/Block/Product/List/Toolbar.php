...
class Mage_Catalog_Block_Product_List_Toolbar extends Mage_Page_Block_Html_Pager
{
    ...
    protected function _construct()
    {
		...
		//	S:VA	Load list view for mobile browsers
		$mode			=	Mage::getStoreConfig('catalog/frontend/list_mode');
		/*$mobileTheme	=	Mage::getSingleton('core/design_package')->getTheme('template');
		//if(Mage::helper('function')->isMobile()){
		if($mobileTheme=='imobile'){
			$mode	=	'list-grid';
		}*/
		//	E:VA
	...
	/* Function added by Hitesh Agarwal to allow relevance sort to have the current page info*/
	public function getCurrentOrderUrl($order, $direction)
    {
        if (is_null($order)) {
            $order = $this->getCurrentOrder() ? $this->getCurrentOrder() : $this->_availableOrder[0];
        }
        return $this->getPagerUrl(array(
            $this->getOrderVarName()=>$order,
            $this->getDirectionVarName()=>$direction,
            $this->getPageVarName() => $this->getCurrentPage()
        ));
    }
