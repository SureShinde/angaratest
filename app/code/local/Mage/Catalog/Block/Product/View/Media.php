...
class Mage_Catalog_Block_Product_View_Media extends Mage_Catalog_Block_Product_View_Abstract
{
	...
	/*
		//	S:VA	Get matching band text based on product type
		//	return string
	*/
	public function getMatchingBandText(){
		$product	=	$this->getProduct();
		if($product->getJewelryType()){
			$jewelryType 	= 	$product->getAttributeText('jewelry_type');					//	Jewelry Type
			if($jewelryType=='Ring'){
				return 'Matching Bands';
			}elseif($jewelryType=='Earrings'){
				return 'Matching Pendants';
			}elseif($jewelryType=='Pendant'){
				return 'Matching Earrings';
			}else{
				return 'Matching Bands';	
			}
		}else{
