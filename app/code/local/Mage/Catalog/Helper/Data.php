...
class Mage_Catalog_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function getBreadcrumbPath()
    {
        if (!$this->_categoryPath) {
            ...
            if ($this->getProduct()) {
                $path['product'] = array('label'=>$this->getProduct()->getShortDescription());
            }
    ...
    public function getCategory()
    {
		...
		/* Commented by Hitesh as it was giving an error
        //return Mage::registry('current_category'); // commented by anil jain
		// start new code
		//
        // If there is a current category then return it (currently default behaviour)
        $current_category = Mage::registry('current_category');
        if ($current_category) return $current_category;
        //
        // If there is a current product use that to provide a
        // current category
        $current_product = $this->getProduct();
        if ($current_product){
            $categories = $current_product->getCategoryCollection()->load();
