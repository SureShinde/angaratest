...
class Mage_Catalog_Helper_Product_View extends Mage_Core_Helper_Abstract
{
    ...
    public function initProductLayout($product, $controller)
    {
		...
		//$settings->setCustomDesign("angara/platina");
		//$settings->setCustomDesign("angara/default");
		
		if($settings->getCustomDesign()){
			$design->applyCustomDesign($settings->getCustomDesign());
		}
		else{
			//if(Mage::getSingleton('customer/session')->getProductTheme()){
				//$design->applyCustomDesign("angara/".Mage::getSingleton('customer/session')->getProductTheme());
			//}
			//else{
				//$design->applyCustomDesign("angara/default");
			//}
			//	Changed theme in IE 7 Starts by Vaseem
			if (stristr($_SERVER['HTTP_USER_AGENT'], "msie 7")){
				$design->applyCustomDesign("angara/default");
			}else{
				//$design->applyCustomDesign("angara/goldmine");
			}
		}
    ...
    public function prepareAndRender($productId, $controller, $params = null)
    {
		...
		/* Setup Build your own product */
		if($product->getIsBuildYourOwn()){
			$jewelryType = $product->getAttributeText('build_your_own_jewelry_type');
			$byoJewelryModel = Mage::getModel('buildyourown/jewelry_'.$jewelryType);
