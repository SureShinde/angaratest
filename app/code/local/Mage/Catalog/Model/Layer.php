...
class Mage_Catalog_Model_Layer extends Varien_Object
{
    ...
    public function prepareProductCollection($collection)
    {
        $collection
            ...
            ->addFinalPrice()
            ->addTaxPercents()
			->addAttributeToSelect('white_gold_image')
			->addAttributeToSelect('yellow_gold_image')
			->addAttributeToSelect('rose_gold_image')
			->addAttributeToSelect('vendor_lead_time')
			->addAttributeToSelect('is_build_your_own')
			->addAttributeToSelect('default_center_diamond_size')		//	S:VA
			//->addAttributeToSelect('build_your_own_size_to')
			//->addAttributeToSelect('build_your_own_size_from')
			/*->addAttributeToSelect('min_price')
			->addAttributeToSelect('max_price')*/
			//->addAttributeToSelect('min_variation_price')
			//->addAttributeToSelect('max_variation_price')
			//->addAttributeToSelect('new_arrival')
			//->addAttributeToSelect('alternate_image')
			//->addAttributeToSelect('vendor_lead_time')
			//->addAttributeToSelect('video')
			->addAttributeToSelect('description')
			->addAttributeToSelect('jewelry_styles')
			//->addStoreFilter()
            ->addUrlRewrite($this->getCurrentCategory()->getId());
			
		$byoJewelryModel = Mage::registry('byoJewelryModel');	
		if($byoJewelryModel && $byoJewelryModel->hasDiamondSelected()){
			$diamond = $byoJewelryModel->getDiamondSelected();
			$diamondSize = $diamond['diamond']['size'];
			$diamondShape = $diamond['diamond']['shape'];
			
			if($diamondSize){
				//	S:VA
				/*if($diamondSize <= '1.00'){
					$collection->addAttributeToFilter('build_your_own_size_from', array('lteq' => $diamondSize));
					$collection->addAttributeToFilter('default_center_diamond_size', array('eq' => '6388'));		//	Up To 1 Carat
				}else{
					$collection->addAttributeToFilter('build_your_own_size_to', array('gt' => $diamondSize));
					$collection->addAttributeToFilter('default_center_diamond_size', array('eq' => '6384'));		//	Greater Than 1 Carat
				}*/
				$collection->addAttributeToFilter('build_your_own_size_from', array('lteq' => $diamondSize));
				$collection->addAttributeToFilter('build_your_own_size_to', array('gteq' => $diamondSize));
			}
			if($diamondShape){
				$shapeDetails = array(
					'Round' => '1658',
				...
				$collection
						->addAttributeToFilter('stone1_shape', array('eq' => $shapeDetails[$diamondShape]));
			}
		}
