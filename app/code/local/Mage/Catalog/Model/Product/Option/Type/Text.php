...
class Mage_Catalog_Model_Product_Option_Type_Text extends Mage_Catalog_Model_Product_Option_Type_Default
{
    ...
    {
        //modified by anil jain to manage prices for BYOJ module in case of percentage
		//start: anil jain
		if(isset($_REQUEST['product_engraving_type'])){
			$_SESSION['product_engraving_type'] = $_REQUEST['product_engraving_type'];
		}
		//end: anil jain
       ...
       // Check maximal length limit
		...
		//start: anil jain
		if(trim($value)!= ''){	
			if(isset($_SESSION['product_engraving_type']) && !empty($_SESSION['product_engraving_type'])){				
				$value = $value.' ('.$_SESSION['product_engraving_type'].')';
			}
		}
		//end: anil jain
