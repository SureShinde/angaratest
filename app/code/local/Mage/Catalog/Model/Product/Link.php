...
class Mage_Catalog_Model_Product_Link extends Mage_Core_Model_Abstract
{
	...
	const LINK_TYPE_MATCHINGBAND	= 6;
	...
	public function useMatchingbandLinks()
    {
        $this->setLinkTypeId(self::LINK_TYPE_MATCHINGBAND);
        return $this;
    }
    ...
    public function saveProductRelations($product)
    {
		...
		$data = $product->getMatchingbandLinkData();
        if (!is_null($data)) {
            $this->_getResource()->saveProductLinks($product, $data, self::LINK_TYPE_MATCHINGBAND);
        }
		$data = $product->getGemstonecolorLinkData();
        if (!is_null($data)) {
            $this->_getResource()->saveProductLinks($product, $data, Angara_Gemstonecolor_Block_Adminhtml_Catalog_Product_Edit_Tab_Gemstonecolor::LINK_TYPE_GEMSTONECOLOR);
        }
        return $this;
    }
