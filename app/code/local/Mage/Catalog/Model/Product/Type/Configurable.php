...
class Mage_Catalog_Model_Product_Type_Configurable extends Mage_Catalog_Model_Product_Type_Abstract
{
     ...
     * @return array
     */
    public function getUsedProducts_old($requiredAttributeIds = null, $product = null)
    {
        Varien_Profiler::start('CONFIGURABLE:'.__METHOD__);
	...
	public function getUsedProducts($requiredAttributeIds = null, $product = null)
    {
        ...
        if (!$this->getProduct($product)->hasData($this->_usedProducts)) {
            ...
            if (!Mage::app()->getStore()->isAdmin() && $product) {
                if (!is_dir(Mage::getBaseDir('var') . DS . 'associated')) {
                    @mkdir(Mage::getBaseDir('var') . DS . 'associated', 0777);
                }
 
                $file_cache = Mage::getBaseDir('var') . DS . 'associated' . DS . $product->getSku() . '~' . $product->getId() . '.php';
				if(is_file($file_cache) && strtotime($product->getUpdatedAt()) >= filemtime($file_cache)){
					/* echo  strtotime($product->getUpdatedAt()).'==='.filemtime($file_cache);die('asheesh_die'); */
					unlink($file_cache);
				}
				$use_cache = true;
            }
     ...
     * @return bool
     */
    public function isSalable_old($product = null)
    {
	...
	public function isSalable($product = null)
    {
        ...
        if ($salable !== false) {
            ...
            if (!Mage::app()->getStore()->isAdmin() && $product) {
				...
				$child = $collection->getFirstItem();
                if ($child && $child->getId()) {
					if ($child->hasData('is_salable')) {
						$salable = $child->getData('is_salable');
					} else {
						foreach ($this->getUsedProducts(null, $product) as $child) {
							if ($child->isSalable()) {
								$salable = true;
        ...
        if ($attributesOption = $this->getProduct($product)->getCustomOption('attributes')) {
            ...
            foreach ($data as $attributeId => $attributeValue) {
                if (isset($usedAttributes[$attributeId])) {
                    ...
                    $attributes[] = array('code'=>$code ,'label'=>$label, 'value'=>$value, 'value_id'=>$valueId);
                }
            }
        }
        Varien_Profiler::stop('CONFIGURABLE:'.__METHOD__);
