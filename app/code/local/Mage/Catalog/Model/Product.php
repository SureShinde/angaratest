...
//        return $result;
     ...
     * @return Mage_Catalog_Model_Product
     */
    /*public function afterCommitCallback() // removed from magento 1.7
    {
 ...
 */
    ...
    public function getCrossSellLinkCollection()
    {
	 ...
	 /**
     * Retrieve array of matching band products
     *
     * @return array
     */
    public function getMatchingbandProducts()
    {
        if (!$this->hasMatchingbandProducts()) {
            $products = array();
            foreach ($this->getMatchingbandProductCollection() as $product) {
                $products[] = $product;
            }
            $this->setMatchingbandProducts($products);
        }
        return $this->getData('matchingband_products');
    }
	
	public function getGemstonecolorProducts()
    {
        if (!$this->hasGemstonecolorProducts()) {
            $products = array();
            foreach ($this->getGemstonecolorProductCollection() as $product) {
                $products[] = $product;
            }
            $this->setGemstonecolorProducts($products);
        }
        return $this->getData('gemstonecolor_products');
    }

    /**
     * Retrieve Matchingband products identifiers
     *
     * @return array
     */
    public function getMatchingbandProductIds()
    {
        if (!$this->hasMatchingbandProductIds()) {
            $ids = array();
            foreach ($this->getMatchingbandProducts() as $product) {
                $ids[] = $product->getId();
            }
            $this->setMatchingbandProductIds($ids);
        }
        return $this->getData('matchingband_product_ids');
    }
	
	public function getGemstonecolorProductIds()
    {
        if (!$this->hasGemstonecolorProductIds()) {
            $ids = array();
            foreach ($this->getGemstonecolorProducts() as $product) {
                $ids[] = $product->getId();
            }
            $this->setGemstonecolorProductIds($ids);
        }
        return $this->getData('gemstonecolor_product_ids');
    }

    /**
     * Retrieve collection Matchingband product
     *
     * @return Mage_Catalog_Model_Resource_Product_Link_Product_Collection
     */
    public function getMatchingbandProductCollection()
    {
        $collection = $this->getLinkInstance()->useMatchingbandLinks()
            ->getProductCollection()
            ->setIsStrongMode();
        $collection->setProduct($this);
        return $collection;
	...
	public function getGemstonecolorProductCollection()
    {
        $collection = $this->getLinkInstance()->setLinkTypeId(Angara_Gemstonecolor_Block_Adminhtml_Catalog_Product_Edit_Tab_Gemstonecolor::LINK_TYPE_GEMSTONECOLOR)
            ->getProductCollection()
            ->setIsStrongMode();
        $collection->setProduct($this);
        return $collection;
	...
	public function getGemstonecolorProductCollectionOnList($id = 0,$list)
    {
		if(!$id) $id = $this->getId();
		if(!$id) return array();
		
		$tag = 'GEMSTONE_PRODUCT_'.Mage::getModel('countrymapping/country')->getCountryParamCode().'_'.$this->getStoreId().'_'.$this->getId();
		$data = '';
		if(Mage::app()->useCache('gemstonecache'))
			$data = Mage::app()->loadCache($tag);
		if(strlen($data) == 0) {
			/* Skip product flat tables for our use */
			/* Start-flat:Asheesh */
			$className = (string)Mage::getConfig()->getNode(Mage_Catalog_Helper_Product_Flat::XML_PATH_FLAT_FLAG);
            $flagObject = Mage::getSingleton($className);
			$builtData = $flagObject->isStoreBuilt($this->getStoreId());
			$flagObject->setStoreBuilt($this->getStoreId(),0);
			/* Continue-flat:Asheesh */
			$collection = $this->getGemstonecolorProductCollection()
				     ->addAttributeToSelect(array(
											'short_description',
											'description',
											'filterable_stone_names',
											'yellow_gold_image',
											'white_gold_image',
											'rose_gold_image',
											'image',
					...
					;
			foreach($collection as $gemproduct){
				if($gemproduct->isSalable()) {
					$gemstoneType = $gemproduct->getAttributeText('filterable_stone_names');
					...
					if(strlen($gemstoneType) > 0) {
						$image = $list->helper('catalog/image')->init($gemproduct, 'small_image')->resize(220);
						$gemproduct->setData('title',$gemTitle);
						$gemproduct->setData('gemstone_type',$gemstoneType);
						if(Mage::registry('current_category')) {
							Mage::register('current_temp_category', Mage::registry('current_category'));
							Mage::unregister('current_category');
						}
						$gemproduct->setData('url',$gemproduct->getUrlInStore());
						
						
						if($gemproduct->getTypeId() == 'configurable') {
							$gemproduct->setData('price_html',$list->getPriceHtml($gemproduct, true, '-similar'));
						} else {
							$gemproduct->setData('price_html',$list->getPriceHtml($gemproduct, true, '-gemstone'));
						}
						if(Mage::registry('current_temp_category')) {
							Mage::register('current_category', Mage::registry('current_temp_category'));
							Mage::unregister('current_temp_category');
						}
						
						$gemproduct->setData('image_url',$image->__toString());
						/* $toggleImage	=	Mage::helper('function')->getToggleImage($gemproduct); */
						if($gemproduct->hasAlternateImage() && $gemproduct->getAlternateImage() && $gemproduct->getAlternateImage()!='no_selection') {
							$gemproduct->setData('toggle',$list->helper('catalog/image')->init($gemproduct, 'thumbnail', $gemproduct->getAlternateImage())->resize(220)->__toString());
						}
						$key = md5('getReviewsSummaryHtmlShort'.$gemproduct->getId());
						if($ReviewsSummaryHtmlShortGem = Mage::helper('helloworld')->getDataFromCache($key)) {
						} else {
							$ReviewsSummaryHtmlShortGem = $list->getReviewsSummaryHtml($gemproduct, 'short');
							Mage::helper('helloworld')->setKey($key)->setData($ReviewsSummaryHtmlShortGem)->saveDataInCache();
						}
						$gemproduct->setData('review_html',$ReviewsSummaryHtmlShortGem);
						$data[] = $gemproduct->getData();
					}
				}
			}
			...
			if(Mage::app()->useCache('gemstonecache'))
				Mage::app()->saveCache($data, $tag, array('GEMSTONES_CACHE'));
			/* Continue-flat:Asheesh */
			/* Set previous value again after our use. */
			$flagObject->setStoreBuilt($this->getStoreId(),$builtData);	
			/* End-flat:Asheesh */
		}
    ...
    /**
     * Retrieve collection Matchingband link
     *
     * @return Mage_Catalog_Model_Resource_Product_Link_Collection
     */
    public function getMatchingbandLinkCollection()
    {
        $collection = $this->getLinkInstance()->useMatchingbandLinks()
            ->getLinkCollection();
        $collection->setProduct($this);
	...
	public function getGemstonecolorLinkCollection()
    {
        $collection = $this->getLinkInstance()->setLinkTypeId(Angara_Gemstonecolor_Block_Adminhtml_Catalog_Product_Edit_Tab_Gemstonecolor::LINK_TYPE_GEMSTONECOLOR)
            ->getLinkCollection();
        $collection->setProduct($this);
 ...
 */
    ...
    {
		$attr = $this->getResource()
            ->getAttribute($attributeCode);
        if($attr && $attr->getId() && $attr->getSource()) {
			return $attr->getSource()->getOptionText($this->getData($attributeCode));
		} else {
     ...
     */
    /*protected function _substractQtyFromQuotes() // removed from magento 1.7
    {
	...
	public function getDiscountToDate(){
		...
		// hard coding active catalog price rule [There should be single catalog price rule that will be active at a time]
				/* fetching Products having catalog price rule active */
		/*if(!isset($this->productIdsArray)){
			  $currentRule = NULL;
			  $rules = Mage::getModel('catalogrule/rule')->getCollection();
			  foreach($rules as $rule){
				  $this->ruleData = $rule->getdata();
				  if($this->ruleData['is_active']){
			  ...
			  if($currentRule){
				  $this->ruleData = $currentRule->getData();
				  $this->productIdsArray = $currentRule->getMatchingProductIds();
			  }
		}
		
		if(in_array( isset($this->ruleData['to_date'])){
			return date('j/n/Y',strtotime($this->ruleData['to_date']));
		}
		else{
			return false;
		}
	...
	public function getCatalogRule(){
		$rule = array();
		$rule['is_active'] = true;
		$rule['to_date'] = '9/15/2011';	// mm/dd/yy
		$rule['discount'] = '15%';
		return $rule;
	}
	public function isUserFromSearch() {
		$lastvisitedurl = Mage::helper('core/http')->getHttpReferer() ;	
		if(stripos($lastvisitedurl, 'showdeal') == FALSE)
		{
			return stripos($lastvisitedurl, "catalogsearch/result") !== FALSE; 
		}
		else 
		{
		return false;
	...
	public function getByoModel(){
		$byoJewelryType = $this->getAttributeText('build_your_own_jewelry_type');
		$byoModel = Mage::getModel('buildyourown/jewelry_'.$byoJewelryType);
		if($byoModel)
			return $byoModel;
		else
			Mage::throwException(Mage::helper('catalog')->__('The BYO model is not defined.'));
	}
}
