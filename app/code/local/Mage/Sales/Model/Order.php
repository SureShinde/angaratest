...
class Mage_Sales_Model_Order extends Mage_Sales_Model_Abstract
{
    ...
    public function addStatusHistoryComment($comment, $status = false)
    {
		...
		// start code: added by anil jain - 31-03-2012
		...
		// end code: added by anil jain - 31-03-2012
    ...
    public function queueNewOrderEmail($forceMode = false)
    {
		...
		$buttonGuestUrl = 'https://www.angara.com/newsletter/subscriber/emailSubscribe/email/'.$this->getBillingAddress()->getEmail().'/location/OrderEmail/order_id/'.$this->getRealOrderId().'/';
		
		$buttonCustomerUrl = 'https://www.angara.com/newsletter/subscriber/emailSubscribe/email/'.$this->getCustomerEmail().'/location/OrderEmail/order_id/'.$this->getRealOrderId().'/';
		
        // Retrieve corresponding email template id and customer name
        if ($this->getCustomerIsGuest()) {
            $templateId = Mage::getStoreConfig(self::XML_PATH_EMAIL_GUEST_TEMPLATE, $storeId);
            $customerName = $this->getBillingAddress()->getName();
            $subscriber = Mage::getModel('newsletter/subscriber')->loadByEmail($this->getBillingAddress()->getEmail());

            if (!$subscriber->getId()) {
                 $subs_html='<tr>
                                 <td style="padding:10px 10px;" align="center">
                                     <div style="background:#f6f6f6;padding-top:20px;padding-bottom:20px;border:1px solid #E0E0E0;">
                                      <div style="padding-bottom:20px;background:#f6f6f6;color:#5c5c5c;">Want to be the first to know about exclusive offers, new arrivals and more?</div>
                                      <div style="height:40px;width:150px;background:#555555;">
                                       <a href="'.$buttonGuestUrl.'" style=" text-align:center; color:#fff; text-decoration:none; font-size:16px; display:block;line-height:40px;">Subscribe</a>
                                      </div>
                                     </div>
                                 </td>
                            </tr>';
                 ...
                 $subs_html='<tr>
                                 <td style="padding:10px 10px;" align="center">
                                     <div style="background:#f6f6f6;padding-top:20px;padding-bottom:20px;border:1px solid #E0E0E0;">
                                      <div style="padding-bottom:20px;background:#f6f6f6;color:#5c5c5c;">Want to be the first to know about exclusive offers, new arrivals and more?</div>
                                      <div style="height:40px;width:150px;background:#555555;">
                                       <a href="'.$buttonCustomerUrl.'" style=" text-align:center; color:#fff; text-decoration:none; font-size:16px; display:block;line-height:40px;">Subscribe</a>
                                      </div>
                                     </div>
                                 </td>
                            </tr>';
		...
		// Start Order PDF generation code
		
		/*$customer_order_id = $this->getRealOrderId();
		$order_pdf_fname = $customer_order_id.'.pdf';
		
		$file_name = $dir.DS.'var'.DS.'gift_order_pdf'.DS.''.$order_pdf_fname; //file path
		
		$fh = fopen($file_name, 'w');
		fwrite($fh, $str);
		fclose($fh);
		
		$cartArr = $this->getAllItems();
		...
		foreach($cartArr as $item){
			if($item->getProduct()->getPrice() <= 0 || $item->getProduct()->getSku()=='INS001' || $item->getProduct()->getSku()=='JA0050') {
				continue;
			}
			$_product = $product->load($item->getProductId());
			//$prod_arr[$p]['pname'] = $item->getProduct()->getName();
			$prod_arr[$p]['pname'] = $_product->getName();
			$mothersflag = 0;
			$customjflag = 0;
			$imgurl = '';
			$categoryIds = $_product->getCategoryIds();
			$customjflag = $_product->getCustomj();
			...
			foreach($categoryIds as $categoryId) {
				...
				$tmpurl = $category->getUrlPath();
				if($tmpurl == "mothers-jewelry.html"){
					$mothersflag = 1;
				}
			}			
			...
			if($mothersflag == 1){								
				...
				if (!in_array($item->getProduct()->getData('sku'), $set_prod_arr)) // skip mother type image for set products
				{
					$imgurl = $home_url."media/catalog/product/images/mothers/cartproducts/" . $cartitemid . ".png";	
				}else{
					$imgurl = $_product->getImageUrl();
				}				
			}else if($customjflag == 1){				
				$imgurl = $home_url.Mage::getBlockSingleton('hprcv/hprcv')->getrootpath() . "cartimages/" . $cartitemid . ".png";				
			}else{	
				$imgurl = $_product->getImageUrl();
			}
			$prod_arr[$p]['pimage'] = $imgurl;
			$p++;
		}*/
		...
		/*if(isset($prod_arr)){
			$pdf_html='<body style="background:url(http://www.angara.com/skin/frontend/angara/default/images/vertual-gift-pdf-bg.jpg) no-repeat;">';
			for($i=0;$i<count($prod_arr);$i++){
			$pdf_html.='<div style="width:100%;height:100%;">
			  <div>
				<div style="width:295px; height:295px; padding-top:383px; margin:0 auto"><img src="'.$prod_arr[$i]['pimage'].'" height="290" width="290"/></div>
				<div style="width:100%; height:40px; padding-top:15px; color:#ac1f15; font:bold 16px Arial, Helvetica, sans-serif; text-align:center">
				'.$prod_arr[$i]['pname'].'</div>
			  </div>
			</div>
			<br />';
			}
		...
		// commented code for order pdf in email attachment
		...
		file_put_contents($file_name, $pdf_content); // save pdf file		
		*/		
		//Mage::log('Path: '.$file_name, null, 'order_pdf_log.log');
		// End Order PDF generation code
...
/*********************** STATUSES ***************************/
	...
	// Start Angara Modification
	public function getTotalOrderByCustomer()
	{
		$customer_email = $this->getCustomerEmail();
		$db = Mage::getSingleton('core/resource')->getConnection('core_write');
		//$sql = "SELECT count(*) as tot FROM sales_flat_order WHERE customer_email='".trim($customer_email)."'";
		$sql = "SELECT GROUP_CONCAT(sales_flat_order.increment_id) as orderlist,count(*) as tot FROM sales_flat_order WHERE customer_email='".trim($customer_email)."'";
		$result = $db->query($sql);
		$rows = array();
		$rows = $result->fetchAll(PDO::FETCH_ASSOC);
		$total_order_by_customer = '1st Order';
		if(count($rows)>0){			
			if($rows[0]['tot']<=1){
				$total_order_by_customer = '1st Order';
			}else{
				$order_arr = explode(',',$rows[0]['orderlist']);
				$order_list = '';
				for($x=0;$x<count($order_arr);$x++){
					if($x<count($order_arr)-1){	
						$order_list.=$order_arr[$x].', ';
					}else{
						$order_list.=$order_arr[$x];
					}
				}
				$total_order_by_customer = $rows[0]['tot'].' ('.$order_list.')';
			}
		}else{
			$total_order_by_customer = '1st Order';
		}
		return $total_order_by_customer;
	}
	// End Angara Modification
}
