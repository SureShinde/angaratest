...
class Mage_Sales_Model_Quote extends Mage_Core_Model_Abstract
{
    ...
    public function addProductAdvanced(Mage_Catalog_Model_Product $product, $request = null, $processMode = null)
    {
        ...
        foreach ($cartCandidates as $candidate) {
			...
			// Angara Modification Start - added by anil			
			...
			if(isset($cartItemDetail['easyopt'])){
				...
				$NoOfInstallment = $no_of_installment + 1;				
				/*if($no_of_installment>0){
					$item->setCustomPrice($installment_amount);
					$item->setOriginalCustomPrice($installment_amount);
			...
			$item->setNoOfInstallment($NoOfInstallment);			
			$item->setEasyPayInstallments($NoOfInstallment);		//	S:VA	Added for saving data in sales_flat_quote_item table			
	   		// Angara Modification End - added by anil
            $items[] = $item;
