...
class Mage_Sales_Model_Order_Invoice extends Mage_Sales_Model_Abstract
{
    ...
    public function sendEmail($notifyCustomer = true, $comment = '')
    {
		...
		//	S:VA 	show custom variable in invoice email
		$orderDateArray		=	explode(' ',$order->getCreatedAt());
		//$orderDate			=	$orderDateArray[0];
		$orderDate			=	date('m-d-Y', strtotime($orderDateArray[0]));
		$shippingTitle		=	Mage::helper('function')->getShipmentTitle($order);
		if($shippingTitle==''){
        ...
        $mailer->setTemplateParams(array(
				...
				'orderDate'    => $orderDate,			//	S:VA to show custom variable in invoice email
				'shipment_title'=>$shippingTitle
            )
        );
        $mailer->send();
    ...
    public function sendUpdateEmail($notifyCustomer = true, $comment = '')
    {
		...
		//	Added by Vaseem to show custom variable in invoice email
		$orderDateArray		=	explode(' ',$order->getCreatedAt());
		$orderDate			=	$orderDateArray[0];
        ...
        $mailer->setTemplateParams(array(
                ...
                'comment'      => $comment,
                'billing'      => $order->getBillingAddress(),
				'orderDate'    => $orderDate			//	Added by Vaseem to show custom variable in invoice email
            )
        );
        $mailer->send();
