...
class Mage_ImportExport_Model_Import_Entity_Product extends Mage_ImportExport_Model_Import_Entity_Abstract
{
	...
	const ERROR_INVALID_UTF8_ENCODING        = 'invalidEncoding';

	protected $_allProductIds = array();
	
	protected $_allLinkProductIds = array();
    ...
    protected $_linkNameToId = array(
		...
		'_links_matchingband_' => Mage_Catalog_Model_Product_Link::LINK_TYPE_MATCHINGBAND,
		'_links_gemstonecolor_' => Angara_Gemstonecolor_Block_Adminhtml_Catalog_Product_Edit_Tab_Gemstonecolor::LINK_TYPE_GEMSTONECOLOR,
    ...
    protected $_messageTemplates = array(
		...
		self::ERROR_INVALID_UTF8_ENCODING        => 'Invalid character encoding(strings must be in UTF-8 encoded) in either short_decription/meta_keyword/meta_title column'
    );
    ...
    protected $_particularAttributes = array(
		...
		'_links_matchingband_sku', '_links_matchingband_position',
		'_links_gemstonecolor_sku', '_links_gemstonecolor_position'
    );
    ...
    protected $_imagesArrayKeys = array(
        '_media_image', 'image', 'small_image', 'thumbnail' , 'alternate_image'
    );
	 ...
	 * @author Asheesh
	 */
	protected function _isUtf8Encoded(array $rowData, $rowNum)
	{
		$withoutEncodedData = Mage::helper('core')->jsonEncode($rowData);
		foreach($rowData as $name=>$value) {
			if(!mb_detect_encoding($value, 'UTF-8', true)) {
				$this->addRowError(self::ERROR_INVALID_UTF8_ENCODING, $rowNum);
	...
	/**
	 * Recurssively encode array data into utf8 encoding
	 *
	 * @param array $d
     * @return array
	 * @author Asheesh
	 */
	protected function utf8EncodeAng($d) {
		if (is_array($d)) {
			foreach ($d as $k => $v) {
				$d[$k] = $this->utf8EncodeAng($v);
			}
		} else if (is_string ($d)) {
        ...
        while ($bunch = $this->_dataSourceModel->getNextBunch()) {
            ...
            foreach ($bunch as $rowNum => $rowData) {
                ...
                if (self::SCOPE_DEFAULT == $this->getRowScope($rowData)) {
					...
					// start new code added by anil for remove old product options before updating by new product csv
					/*if ($productId) {
						$this->_connection->query("DELETE FROM catalog_product_option where product_id=".$productId);
					}*/
					// end new code added by anil for remove old product options before updating by new product csv	
    ...
    protected function _saveLinks()
    {
        ...
        while ($bunch = $this->_dataSourceModel->getNextBunch()) {
            ...
            if (Mage_ImportExport_Model_Import::BEHAVIOR_APPEND != $this->getBehavior() && $productIds && $linkRows) {
                $productIds = array_diff($productIds,$this->_allLinkProductIds);
				
				 $where = $adapter->quoteInto('product_id IN (?)', array_unique($productIds)).
							$this->_connection->quoteInto(' AND link_type_id IN (?)', array_unique($specificLinkIds));
				$adapter->delete(
                    $mainTable,
                    $where
                );
				$this->_allLinkProductIds = array_merge($this->_allLinkProductIds,$productIds);
				$this->_allLinkProductIds = array_unique($this->_allLinkProductIds);
            }
            if ($linkRows) {
    ...
    protected function _saveProductCategories(array $categoriesData)
    {
        ...
        if ($categoriesData) {
            ...
            if (Mage_ImportExport_Model_Import::BEHAVIOR_APPEND != $this->getBehavior()) {
                $delProductId = array_diff($delProductId,$this->_allProductIds);
				$this->_connection->delete(
                    $tableName,
                    $this->_connection->quoteInto('product_id IN (?)', $delProductId)
                );
				
				$this->_allProductIds = array_merge($this->_allProductIds,$delProductId);
				$this->_allProductIds = array_unique($this->_allProductIds);
            }
            if ($categoriesIn) {
    ...
    protected function _saveProducts()
    {
        ...
        while ($bunch = $this->_dataSourceModel->getNextBunch()) {
            ...
            foreach ($bunch as $rowNum => $rowData) {
                $this->_filterRowData($rowData);
                if (!$this->validateRow($rowData, $rowNum, 0)) {
                    continue;
                }
    ...
    protected function _prepareAttributes($rowData, $rowScope, $attributes, $rowSku, $rowStore)
    {
        ...
        foreach ($rowData as $attrCode => $attrValue) {
            ...
            if ('datetime' == $attribute->getBackendType() && strtotime($attrValue)) {
                ...
                if (empty($attrValue)) {
                    $attrValue = $product->formatUrlKey($product->getName().' '.$rowSku);
                }
            } elseif ($backModel) {
                $attribute->getBackend()->beforeSave($product);
                $attrValue = $product->getData($attribute->getAttributeCode());
     ...
     * @return boolean
     */
    public function validateRow(array $rowData, $rowNum, $import = 1)
    {
		...
		if($import) 
			$this->_isUtf8Encoded($rowData, $rowNum);
