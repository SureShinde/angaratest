...
abstract class Mage_Core_Controller_Varien_Action
{
    ...
    public function preDispatch()
    {
		...
		//	S:VA
		if($this->getRequest()->getParam('resetChannel') == 'true'){
			$channel = Mage::getModel('promotions/channel')->getChannelByUrl(Mage::app()->getRequest()->getRequestUri());
			Mage::getSingleton('core/session')->setVisitorChannel($channel);
			Mage::getModel('promotions/offer')->setDefaultOffer();
			Mage::getSingleton('core/session')->setPromotionApplied(false);
		}
		
		if ($country = (string) $this->getRequest()->getParam('switchCountry')) {
				//Mage::app()->getStore()->setCurrentCurrencyCode($renderedCurrency);
				...
				//$requestUrl = empty($requestUrl) ? Mage::getBaseUrl() : $requestUrl;
				//$requestUrl = str_replace('switchCountry','newloc',$requestUrl);
