...
class Mage_Core_Model_Store extends Mage_Core_Model_Abstract
{
	...
	*/
    ...
    {
        $codes = $this->getData('available_country_codes');
        if (is_null($codes)) {
            $codes = explode(',', $this->getConfig(Angara_Countrymapping_Model_Country::XML_PATH_COUNTRY_ALLOW));
            // add base currency, if it is not in allowed currencies
            $baseCountryCode = $this->getConfig(Angara_Countrymapping_Model_Country::XML_PATH_COUNTRY_BASE);
            if (!in_array($baseCountryCode, $codes)) {
                $codes[] = $baseCountryCode;
            }
            $this->setData('available_country_codes', $codes);
        }
        return $codes;
    }
}
