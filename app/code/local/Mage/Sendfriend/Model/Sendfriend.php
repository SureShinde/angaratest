...
class Mage_Sendfriend_Model_Sendfriend extends Mage_Core_Model_Abstract
{
    ...
    public function send()
    {
		...
		$totalEmailsCount = count($this->getRecipients()->getEmails());
		$isSendCopy = Mage::app()->getRequest()->getParam('is_send_copy');
		$emailCount = 0;
		
        foreach ($this->getRecipients()->getEmails() as $k => $email) {
            $name = $this->getRecipients()->getNames($k);
			$emailCount++;
			/* Send a copy to user if checked */
			if($emailCount == $totalEmailsCount && $isSendCopy){
			...
			try{
				$mailTemplate->sendTransactional(
					...
					array(
						...
						'product_url'   => $this->getProduct()->getProductUrl().'?cid=sm-email-organic&resetChannel=true',
						'message'       => $message,
		...
		/* Subscribe user if checked for updates */
		try{
			$isSubscribe = Mage::app()->getRequest()->getParam('is_subscribed');				
			if($isSubscribe){
				$newsletterModel = Mage::getModel('newsletter/subscriber');
				$isSubscribed = $newsletterModel->loadByEmail($sender['email']);
				
				if(!$isSubscribed->getSubscriberEmail()){
					Mage::getModel('newsletter/subscriber')->subscribe($sender['email']);
					
					$session = Mage::getSingleton('core/session');
					$sessionId = $session->getSessionId(); 
					$mailchimp = Mage::getModel('abandoncartmailchimp/session');		
							$mailchimp->setSessionId($sessionId);
							$mailchimp->setVisitorEmail($sender['email']);												
