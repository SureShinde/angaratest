...
class Mage_SalesRule_Model_Validator extends Mage_Core_Model_Abstract
{
    ...
    public function init($websiteId, $customerGroupId, $couponCode)
    {
        ...
        if (!isset($this->_rules[$key])) {
			...
			//$coupon_code 	= 	Mage::getSingleton('checkout/session')->getQuote()->getData('coupon_code');
			//if ( strtolower($coupon_code) != 'event') {
			/* if(Mage::helper('function')->getExtraDiscount() == '0' ){
				$collection->getSelect()
					->where('main_table.description NOT LIKE ?', "timerCoupon");
			} */
    ...
    public function process(Mage_Sales_Model_Quote_Item_Abstract $item)
    {
        ...
        foreach ($this->_getRules() as $rule) {
            ...
            $baseDiscountAmount=round($baseDiscountAmount);
