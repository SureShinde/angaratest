...
class Mage_SalesRule_Model_Quote_Discount extends Mage_Sales_Model_Quote_Address_Total_Abstract
{
    ...
    public function collect(Mage_Sales_Model_Quote_Address $address)
    {
		...
		//	S:VA	Adding free products to cart
		if(Mage::getSingleton('core/session')->getPromotionApplied() == false){
			Mage::getModel('promotions/offer')->process($quote->getAppliedRuleIds());
		}
    ...
    public function fetch(Mage_Sales_Model_Quote_Address $address)
    {
        ...
        //if ($amount!=0) { // commented to show discount block if coupon not applied.
            $description = $address->getDiscountDescription();
            if (strlen($description)) {
                //$title = Mage::helper('sales')->__('Discount (%s)', $description);
				//	S:VA
				$title = Mage::helper('sales')->__('Promo Code <span class="text-uppercase apricot-text">(%s)</span>', $description);
            } else {
                $title = Mage::helper('sales')->__('Promo Code');
            }
