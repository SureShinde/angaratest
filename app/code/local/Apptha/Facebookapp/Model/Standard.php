 ...
 */
class Apptha_Facebookapp_Model_Standard extends Mage_Paypal_Model_Standard
{
   protected $_code  = Mage_Paypal_Model_Config::METHOD_WPS;
    ...
    {
        $getUrl = Mage::getStoreConfig('facebookapp/general/facebook_app_path'); 
        $orderIncrementId = $this->getCheckout()->getLastRealOrderId();
        $order = Mage::getModel('sales/order')->loadByIncrementId($orderIncrementId);
        $api = Mage::getModel('paypal/api_standard')->setConfigObject($this->getConfig());
        if(strtoupper(Mage::app()->getStore()->getWebsite()->getCode()) == 'FACEBOOK') {
        $api->setOrderId($orderIncrementId)
            ->setCurrencyCode($order->getBaseCurrencyCode())
            //->setPaymentAction()
            ->setOrder($order)
            ->setNotifyUrl(($getUrl.'paypal/ipn/'))
            ->setReturnUrl(($getUrl.'paypal/standard/success'))
            ->setCancelUrl(($getUrl.'paypal/standard/cancel'));
        }
        else {
            $api->setOrderId($orderIncrementId)
            ->setCurrencyCode($order->getBaseCurrencyCode())
