...
class AW_Followupemail_Model_Events
{
	...
	public function newsletterSubscribed($eventData)
	{
		$customer = $eventData;
		$queue = Mage::getResourceModel('followupemail/queue');
		$queue->cancelByEvent($customer->getEmail(),
			AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_NEWSLETTER_SUBSCRIBED);
		//Customer group has been changed event processing
		$ruleIds = Mage::getModel('followupemail/mysql4_rule')
			->getRuleIdsByEventType(AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_NEWSLETTER_SUBSCRIBED);
		if(count($ruleIds)) {
			$params = array(
