...
// the next two lines were moved here from __construct() due to compatibility issue to Magento version 1.2
    ...
    public function thumbnailDirective($construction)
    {
        ...
        $urlTemp = Mage::helper('catalog/image')
                    ->init($product, 'thumbnail', $thumbnail)
                    ->resize($imgDimension);
		$url = Mage::helper('function')->getResizeImageUrl( $urlTemp, $imgDimension, $imgDimension);
		//$url = Mage::getModel('catalog/product_media_config')->getMediaUrl($product->getThumbnail()); 	//	S:VA	Direct image path shown in review emails
        return $url;
    }
