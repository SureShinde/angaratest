...
class AW_Followupemail_Model_Cron
{
    ...
    public function cronJobs()
    {
		...
		//	S:VA	Take server time rather than considering MySql time
		...
		$this->_nowMySQL				=	date("Y-m-d H:i:s", strtotime("-480 minutes", strtotime($this->_nowMySQL)));
		$this->_lastExecTimeMySQL		=	date("Y-m-d H:i:s", strtotime("-480 minutes", strtotime($this->_lastExecTimeMySQL)));
		//Mage::log('_lastExecTimeMySQL '.$this->_lastExecTimeMySQL.' and _nowMySQL '.$this->_nowMySQL, null, 'followup_queue.log', true);
        ...
        try {
			...
			Mage::log('Cron started , last execution time is '.$this->_lastExecTime.' and timeShift is '.$timeShift, null, 'followup_queue.log', true);
		...
		Mage::log('Cron stopped at '.$this->_now , null, 'followup_queue.log', true);
    }
    ...
    {
		//Mage::log('_checkOrderStatusHistory started ', null, 'followup_queue.log', true);
        ...
        while($currentPage <= $_pages) {
			...
			$statusHistoryQuery	=	(string)$statusHistoryCollection->getSelect();
			Mage::log('Get order status history '.$statusHistoryQuery."\n\r", null, 'followup_queue.log', true);
    ...
    protected function _checkAbandonedCarts()
    {
        ...
        $select = $read->select()
            ...
            ->where('q.updated_at > ?', date(AW_Followupemail_Model_Mysql4_Queue::MYSQL_DATETIME_FORMAT,
                                                        $this->_lastExecTime + $timeShift - self::SESSION_TIMEOUT))
            ->where('q.updated_at < ?', date(AW_Followupemail_Model_Mysql4_Queue::MYSQL_DATETIME_FORMAT,
                                                        $this->_now + $timeShift - self::SESSION_TIMEOUT))
