...
class AW_Followupemail_Model_Rule extends Mage_Core_Model_Abstract
{
     ...
     */
    public function _getContent($objects, $templateId)
    {
    ...
    public function process($params, $objects = array())
    {
        ...
        if($this->_isValid || $objects['email']){
            ...
            foreach ($chain as $chainItem) {
				...
				//	S:VA
				$objects['expiration_date'] = date('d-M-Y', strtotime("+60 days"));					
				if($objects['customer_name']){
					$arr 		= 	explode(' ',trim($objects['customer_name']));
					$firstName	=	$arr[0];
					$objects['first_name'] =  $firstName;
				}else{
					$objects['first_name'] =  'Friend';	
				}
				//	E:VA
                ...
                if(!$content = $this->_getContent($objects, $chainItem['TEMPLATE_ID'])) {
					...
					if($this->getId()=='5') {
						Mage::dispatchEvent('order_completion_followup_after', array('orderid'=>$objects['order_id'], 'email'=>$objects['customer_email'], 'sequencenumber'=>$objects['sequence_number'], 'followupobjectid'=> $objects['object_id']));
						//	S:VA
						Mage::log('Email set in queue for Order Id-'.$objects['order_id']. ' and Rule Id-'. $this->getId() , null, 'followup_queue.log', true);
					}
                }
                $sequenceNumber++;
            }
		...
		//	S:VA
		Mage::log('Rule Id-'.$this->getId().' is not valid for Event-'.$this->getEventType().' and Reason-'.$this->_validationMessage , null , 'followup_queue.log',true);
		//Mage::log('process function called from Rule.php and order status is '.$objects['order']->status, null, 'followup_queue.log');
