...
class AW_Rma_Adminhtml_RmaController extends AW_Rma_Adminhtml_AbstractController
{
    ...
    protected function saveAction()
    {
		...
		//echo '<pre>';print_r($_data);		echo '<pre>';print_r($rmaRequest);die;
		...
		//	S:VA	Check if Request type is changed
		if( ( $_data['request_type'] != $rmaRequest->getData('request_type') ) && ($this->getRequest()->getParam('id')) ){
			$appendComments	=	'The Request type has been changed from '. Mage::helper('awrma')->getTypeLabel($rmaRequest->getData('request_type')) .' to '. Mage::helper('awrma')->getTypeLabel($_data['request_type']).'. ';
		}
		//	E:VA
		...
		//	S:VA	Check if Package Opened is changed
		if($_data['package_opened'] != $rmaRequest->getData('package_opened') && ($this->getRequest()->getParam('id'))){
			$appendComments.=	' The Package opened has been changed from '. Mage::getModel('awrma/source_packageopened')->getOptionLabel($rmaRequest->getData('package_opened')).' to '. Mage::getModel('awrma/source_packageopened')->getOptionLabel($_data['package_opened']).'. ';
		}
		//	E:VA
        ...
        if ($_data['order_items'] == array()) {
			...
			$this->_getSession()->addError(Mage::helper('awrma')->__('Please check if the order status is complete or the request should have at least one item.'));
        }
            ...
            if ($this->getRequest()->getParam('comment_text')) {
                ...
                if ($_data['text']) {
                    ...
                    if (!$this->hasErrors()) {
					   ...
					   //	S:VA	-	Modifying the id of person who is posting comments
						$adminSession 	= Mage::getSingleton('admin/session');
						$_data['owner'] = $adminSession->getUser()->getUserId();
						
						if($appendComments){
							$_data['text']	=	$appendComments.$_data['text'];
						}
						
						//	E:VA	-	Modifying the id of person who is posting comments
