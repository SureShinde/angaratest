...
class AW_Rma_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public static function getItemsHtml($rmaRequest, $params = array(), $email)
    {
        $params = array_merge($params, array('itemscount' => $rmaRequest->getOrderItems()));
        return self::getItemsForOrderHtml($rmaRequest->getData('order_id'), 2, '', $params, $email);		//	M:VA
    }
     ...
     * @return array
     */
    public static function getItemsForOrderHtml($orderId, $guestMode, $data, $params = array(), $email)		//	M:VA
    {
        $result = array();
        ...
        if (!($_order->getData() == array())) {
            ...
            foreach ($_orderItems as $_item) {
				//	S:VA
				if($email==1){
					$_item->setData('use_for_email_only', 1);	
				}
				//	E:VA
