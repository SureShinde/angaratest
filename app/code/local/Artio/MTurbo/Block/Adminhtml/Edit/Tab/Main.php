...
class Artio_MTurbo_Block_Adminhtml_Edit_Tab_Main extends Artio_MTurbo_Block_Adminhtml_Edit_Tab_Abstract
{
    ...
    private $_isDefinedInterpret=false;
    ...
    protected function _prepareForm() {
        ...
        $this->_isDefineInterpret = ($config->getInterpret()!='');
    ...
    private function _addAutomaticDownloadFieldset() {
        ...
        $layoutFieldset->addType('html', Artio_MTurbo_Helper_Data::FORM_HTML);
        ...
        if (!is_executable($scriptPath) && !$this->_isDefineInterpret) {
            $layoutFieldset->addField ( 'scriptstate', 'html',
            array ('label' => '<h4>'.$this->getMyHelper()->__('Script state').'</h4>',
                   'code' => '<span style="color:red">'.Mage::helper('mturbo')->__("Script '%s' is not executable. Please change permission or define interpret for downloading script", $scriptPath).'</span>'));
        }

        /* add field for interpret */
        $layoutFieldset->addField('interpret', 'text', array(
            'name'      => 'interpret',
            'label'     => $this->getMyHelper()->__('Interpret for downloading script').':',
            'value'     => ''));
