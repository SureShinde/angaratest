...
class Mage_Customer_Model_Observer
{
    ...
    public function beforeLoadLayout($observer)
    {
        ...
        if(Mage::app()->getRequest()->getParam('email') && Mage::app()->getRequest()->getParam('cid')!='')
        {
          $string=Mage::app()->getRequest()->getParam('cid');
          $arr = explode("-", $string, 2);
        $first = $arr[0];
        if($first=="em")
            Mage::getSingleton('checkout/session')->setEmailFreeShipping(Mage::app()->getRequest()->getParam('email'));
        
       //Mage::log(Mage::getSingleton('checkout/session')->getEmailFreeShipping(),null,"chsess2.log",true); 
