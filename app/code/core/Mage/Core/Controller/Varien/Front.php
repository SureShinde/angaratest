...
class Mage_Core_Controller_Varien_Front extends Varien_Object
{
	...
	/**
     * Auto-redirect to base url (without SID) if the requested url has https request.
     *
     * @param Zend_Controller_Request_Http $request
     */
    protected function _checkHttpUrl($request)
    {
        if (!Mage::isInstalled() || $request->getPost() || $request->isAjax()) {
            return;
        }

        $redirectCode = (int)301;
        if (!$redirectCode) {
            return;
        } elseif ($redirectCode != 301) {
            $redirectCode = 302;
        ...
        $uri = @parse_url($baseUrl);
		$requestUrlScheme = $request->getScheme();	
		$requestUri = $request->getRequestUri() ? $request->getRequestUri() : '/';		
		$redirectRequestUrl	= $uri['scheme'].'://'.$uri['host'].$requestUri;
		if(isset($requestUrlScheme) && $requestUrlScheme != 'http') {
			if(!preg_match('/checkout/i',$requestUri) && !preg_match('/kount/i',$requestUri) && !preg_match('/sharecart/i',$requestUri) && !preg_match('/onepagecheckout/i',$requestUri) && !preg_match('/paypal/i',$requestUri) && !preg_match('/amazon/i',$requestUri)&& !preg_match('/paygate/i',$requestUri) && !preg_match('/googlecheckout/i',$requestUri) && !preg_match('/paypaluk/i',$requestUri)) {
				Mage::app()->getFrontController()->getResponse()
					//->setRedirect($baseUrl, $redirectCode)
					->setRedirect($redirectRequestUrl, $redirectCode)
					->sendResponse();
				exit;
			}
		}
