...
class Ebizmarts_MailChimp_Model_Observer
{
    ...
    public function updateTimeEntry(Varien_Event_Observer $observer)
    {
    ...
    public function updateTimeEntryEmail(Varien_Event_Observer $observer)
    {
        
        $subscriber = $observer->getEvent()->getEmail();
        $sub=Mage::getModel('newsletter/subscriber')->loadByEmail($subscriber);
        
        if($sub->getFreeShipping()==1)
        {
            $sub->setData("mailchimp_sync_delta", '0000-00-00 00:00:00');
             $sub->save();
        }
