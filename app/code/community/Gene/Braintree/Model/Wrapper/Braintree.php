...
class Gene_Braintree_Model_Wrapper_Braintree extends Mage_Core_Model_Abstract
{
    ...
    public function validateCredentialsOnce()
    {
        ...
        if (is_null($this->_validated)) {
            ...
            if (Braintree_Version::get() < 2.32) {
                ...
                else if (!$this->_validateConfiguration()) {
    ...
    public function makeSale($saleArray)
    {
        ...
        //$vault_status=Mage::getStoreConfig('payment/gene_braintree_creditcard/use_vault');
        //if(Mage::helper('function')->isEmiCollection())
        $saleArray['options']['storeInVaultOnSuccess']=true;
        $quote = Mage::getSingleton('checkout/session')->getQuote();
        try{
            $saleArray['creditCard']['cardholderName']=$quote->getPayment()->getCcOwner();
        }catch(Exception $e){
            //
        }
        ...
        if(Mage::helper('function')->isEmiCollection())
        {
            ...
            $token=$sale->transaction->creditCard['token'];
            ...
            foreach ($cartItems as $item)
            {
                if($item->getNoOfInstallment() > 1)
                {
                    ...
                    $planid= Mage::getStoreConfig('payment/gene_braintree_creditcard/plan_id');
                    if(isset($planid))
                    {
                        $result = Braintree_Subscription::create(array(
                      'paymentMethodToken' => $token,
                      'planId' => $planid,
                      'numberOfBillingCycles' => $item->getNoOfInstallment()-1,
                      'price' => $amount,
