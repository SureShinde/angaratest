...
class IllApps_Shipsync_Model_Shipping_Carrier_Fedex_Rate extends IllApps_Shipsync_Model_Shipping_Carrier_Fedex
{
	...
	$rateRequest = $this->_rateRequest;
	   ...
	   // Code Added by Vaseem for 46
	   //echo 'signature value->'.$request->getRequireSignature();
	   	if ($request->getRequireSignature()){ $rateRequest->setRequireSignature('DIRECT'); }
		else { $rateRequest->setRequireSignature('SERVICE_DEFAULT'); }
		// Code Added by Vaseem for 46
	...
	{	    	    
		...
		$i=0; foreach($packages as $package)
		{
                    ...
                    if ($rateRequest->getOrigCountry() != $rateRequest->getDestCountry())
                    {
                        foreach ($package['items'] as $item)
                        {
                            ...
                            $commodities[] = array(
                                    ...
                                    'CustomsValue' => array('Currency' => $this->getCurrencyCode(), 'Amount' => $itemValue)
                                );
                        }
                    }
			...
			$request['RequestedShipment']['RequestedPackageLineItems'][$i]['Dimensions'] = array(
				...
				// Code Added by Vaseem for 46
				//echo 'signature value to pass->'.$rateRequest->getRequireSignature(); 
				if ($rateRequest->getRequireSignature() == 'DIRECT'){
					$request['RequestedShipment']['RequestedPackageLineItems'][$i]['SpecialServicesRequested'] = array(
