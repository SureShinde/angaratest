...
final class Mage
{
	...
	/**
	* Location From IP added by Hitesh
	*/
	static private $_locationFromIP;
	
	/*
	* User Agent added by Hitesh
	*/
	static private $_userAgent;
	...
	public static function isOverwriteCustomImages()
	{
		return false;
	}
	public static function getMemcache()
	{
		if (class_exists('Memcache')) {
			...
			if(!$connect){
				//Mage::log("Could not connect to memcache server", null, 'countrymapping.log');
				return false;
			}else{
				return $memcache;
			}
	...
	{
		if(!empty(self::$_locationFromIP)){
			return self::$_locationFromIP;
		}
		...
		*/
		return false;
		$ip = Mage::helper('core/http')->getRemoteAddr(); // modified by anil jain - 26-12-2012
		
		//$ip = '109.104.105.69';
		if($ip!='UNKNOWN') {
			  //Mage::log('IP Found successfully - '. $ip , null, 'countrymapping.log');	
			  $ch = curl_init();
			  $timeout = 5;
			  //$ip='74.11.95.200';
			  //$ip='59.90.194.72';
			  //$url = 'http://api.hostip.info?format=xml&ip='. $ip;
			  //$url='http://api.hostip.info/get_html.php?ip=59.90.194.72';
			  //US IP- 74.11.95.200
			  //India IP-59.90.194.72
			  //echo $url="http://api.ipinfodb.com/v3/ip-country/?key=". $ipinfoKey."&ip=". $ip . "&format=xml";
			 ...
			 try
			 {
				...
				if($xml->statusCode=='OK' AND $xml->countryCode!='-') {
					...
					//Mage::log('Found countrycode from locastionApi - '.$url.'<==>'. $countryCode , null, 'countrymapping.log');
					self::$_locationFromIP = $countryCode;
			   	    return $countryCode;	
				} else {
					//Mage::log('Not Found countrycode from locationApi - ' , null, 'countrymapping.log');
					return false;
				}		
			 }catch(Exception $e)
			 {
			...
			//Mage::log('UNKNOWN IP - Could not detect IP' , null, 'countrymapping.log');
			return false;
		}
	}
	// Start Angara Modification
	public static function getLoggedInMemberPrice($retailprice = NULL)
	{
		$memberOfferCoupon = Mage::getModel('salesrule/coupon')->load('ANGMEMBER', 'code');
		$memberRule = Mage::getModel('salesrule/rule')->load($memberOfferCoupon->getRuleId());	
		//var_dump($memberRule);exit;
		if($memberRule->getSimpleAction() == 'by_percent'){
			$offerAmt = $memberRule->getDiscountAmount();
			$MemberPriceValue = round((100-$offerAmt)*$retailprice / 100,2);
		}else{
			$MemberPriceValue = $retailprice;
		}
		return $MemberPriceValue;
	}
	
	public static function getLoggedInMemberDiscountVal()
	{
		$memberOfferCoupon = Mage::getModel('salesrule/coupon')->load('ANGMEMBER', 'code');
		$memberRule = Mage::getModel('salesrule/rule')->load($memberOfferCoupon->getRuleId());	
		//var_dump($memberRule);exit;
		if($memberRule->getSimpleAction() == 'by_percent'){
			$DiscountVal = $memberRule->getDiscountAmount();
		}else{
			$DiscountVal = 0;
		}
		return $DiscountVal;
	...
	public static function getUserAgent(){
		if (self::$_userAgent === null) {
			if(!Mage::getSingleton('core/session')->getUserAgent()){
				self::$_userAgent = Angara_Client::parse();
				Mage::getSingleton('core/session')->setUserAgent(self::$_userAgent);
			}
			else{
				self::$_userAgent = Mage::getSingleton('core/session')->getUserAgent();
			}
		}
