...
Product.Config.prototype.reloadPrice = function() {
    ...
    if(childProductId){
		...
		if(typeof(priceChanged) != 'undefined'){
			...
			if(jQuery('.spacing').find('#ultimate-engraving').hasClass('active')){
...
Product.Config.prototype.updateProductImage = function(productId) {
    if(productId && this.config.productImages[productId].length > 0) {
		...
		if($('image')) {
			var currentIndex = jQuery('#moreviews').find('ul li.active:not(.static-view)').index();
			if(currentIndex < 0){
				currentIndex = 0;
			}
						...
						jQuery('<li>').append(
							jQuery('<span>').append(
								jQuery('<img width="63" height="63">')
									...
									//if(iOS==false){
										.attr('onclick','imageswitcher("' + image.url + '")')
									//}
									//.attr('onclick','imageswitcher("' + image.url + '")')		//	Added by Vaseem for image zoom
							).addClass('thumbs')
						).click(function(){
							$('image').src = image.url;
							moreViewClicked();
			...
			jQuery('#moreviews').find('ul li').removeClass('active');
			var iOS = ( navigator.userAgent.match(/(iPad)/g) ? true : false );//iPhone|iPod
			if(iOS==true){
				jQuery('#moreviews').find('ul li:eq('+currentIndex+')').addClass('active');
                        }
                        else{
				eval(jQuery('#moreviews').find('ul li:eq('+currentIndex+')').addClass('active').find('img').attr('onclick'));
                        }
			//jQuery('#moreviews').find('ul li:eq('+currentIndex+')').addClass('active');		//	Added by Vaseem for image zoom
			//eval(jQuery('#moreviews').find('ul li:eq('+currentIndex+')').addClass('active').find('img').attr('onclick'));		//	Added by Vaseem for image zoom
			updateMoreViews();
		}
	}
		...
		beautifyOption = function(optionClickable, optionContainer, option, attributeId, attribute, parent, optionDetailsContainer){
			...
			switch(attribute.code){
				case 'stone1_size':
					...
					var imageWidth = 81 - (5 * (attribute.options.length - stoneSizeImages.length));
					//var imageElement = $('<img width="75" height="75" src="' + url + '" />');
					...
					if(typeof(settings.childProducts[option.products[0]]['total_weight']) != 'undefined'){
						$('<div>').append(optionClickable).append('<span class="user-option-title">'+settings.childProducts[option.products[0]]['total_weight']+'</span>').appendTo(optionContainer);
					}
					else{
					...
					optionDetail.html('<strong>Approximate Carat Weight:</strong> '+settings.childProducts[option.products[0]]['weight_description']+'s.');
					break;
				case 'stone1_grade':
				...
				case 'band_width':
					bandWidthOptions = attribute.options.length;
				 	if(spConfig.getMatchingSimpleProduct()){
						var url = settings.productImages[spConfig.getMatchingSimpleProduct()][0].thumbUrl
					}else{
						var url = settings.productImages[option.products[0]][0].thumbUrl;
					}
					bandWidthAttributeId = attributeId;
					var imageWidth = 81 - (5 * (attribute.options.length - bandWidthImages.length));
					//var imageElement = $('<img width="75" height="75" src="' + url + '" />');
					var imageElement = $('<img width="'+imageWidth+'" height="'+imageWidth+'" src="' + url + '" />').addClass('imgsize');
					...
					optionClickable.append(
						$('<span>').addClass('user-option-image')
						.append(imageElement)
						.append('<span class="user-option-title">'+option.label+'</span>')
					);
					$('<div>').append(optionClickable).appendTo(optionContainer);
					break;
				default:
				...
				beautifyOption(optionClickable, optionContainer, option, attributeId, attribute, parent, optionDetailsContainer);
					optionClickable.click(function(){
						//jQuery(".attribValue option:selected").each(function() {
							//if(jQuery(this).text()!='SELECT RING SIZE'){
...
if(attribute.code!='ring_size'){
		...
		variationsMarkup = function(){
			var variationsContainer = $('<div id="variations-inner-container">').appendTo(container);
			if(typeof(settings.stone1_shape) != 'undefined' && typeof(settings.stone1_name) != 'undefined')
				variationsContainer.addClass('stone-'+settings.stone1_shape.replace(/\W/g,'')+'-'+settings.stone1_name.replace(/\W/g,''));
		...
		getStoneGradeDescription = function(stone, grade){
			...
			switch(stone+grade){
				case 'SapphireA':
					return '<strong>Quality Grade - Good (A): </strong> Top 75% of sapphires in terms of quality.  Dark blue and opaque.  This quality is comparable to that used by mall jewelers and chain stores.';
					break;
				case 'SapphireAA':
				...
				case 'RubyA':
					return '<strong>Quality Grade - Good (A): </strong> Top 75% of rubies in terms of quality. &nbsp;Dark pinkish red and opaque. &nbsp;This quality is comparable to that used by mall jewelers and chain stores.';
					break;
				case 'RubyAA':
				...
				case 'EmeraldA':
					return '<strong>Quality Grade - Good (A): </strong> Top 75% of emeralds in terms of quality. &nbsp;Dark green and opaque. &nbsp;This quality is comparable to that used by mall jewelers and chain stores.';
					break;
				case 'EmeraldAA':
				...
				case 'TanzaniteA':
					return '<strong>Quality Grade - Good (A): </strong> Top 75% of tanzanites in terms of quality. &nbsp;Light violet blue and slightly included. &nbsp;This quality is comparable to that used by mall jewelers and chain stores.';
					break;
				case 'TanzaniteAA':
				...
				case 'AquamarineA':
					return '<strong>Quality Grade - Good (A): </strong> Top 75% of aquamarines in terms of quality. &nbsp;Very light sea blue and moderately included. &nbsp;This quality is comparable to that used by mall jewelers and chain stores.';
					break;
				case 'AquamarineAA':
				...
				case 'DiamondJ I2':
					return '<strong>Quality Grade - (J I2): </strong> Top 50% of diamonds in terms of quality. &nbsp;J Color and I2 Clarity. &nbsp;This quality is comparable to that used by mall jewelers and chain stores.';
					break;
				case 'DiamondI I1':
					return '<strong>Quality Grade - (I I1): </strong> Top 25% of diamonds in terms of quality. &nbsp;I Color and I1 Clarity. &nbsp;This quality is comparable to that used by leading independent/family jewelers.';
					break;
				case 'DiamondH SI2':
					return '<strong>Quality Grade - (H SI2): </strong> Top 10% of diamonds in terms of quality. &nbsp;H Color, SI2 Clarity and exhibits high brilliance. &nbsp;This quality is comparable to that used by the top 5th Avenue or Rodeo Drive Jewelers.';
					break;
				case 'DiamondG-H VS':
					return '<strong>Quality Grade - (G-H VS): </strong> Top 1% of diamonds in terms of quality. &nbsp;Truly exceptional G-H Color, VS Clarity and exhibits very high brilliance. &nbsp;This quality can be found only at the top boutiques in the world.';
					break;
					
				case 'Enhanced Black DiamondA':
					return '<strong>Quality Grade - Good (A): </strong> Top 50% of enhanced black diamond in terms of quality. &nbsp;Opaque in Clarity. &nbsp;This quality is comparable to that used by mall jewelers and chain stores.';
					break;
				case 'Enhanced Black DiamondAA':
					return '<strong>Quality Grade - Better (AA): </strong> Top 25% of enhanced black diamond in terms of quality. &nbsp;Opaque in Clarity. &nbsp;This quality is comparable to that used by leading independent/family jewelers.';
					break;
					
				case 'Enhanced Blue DiamondAA':
					return '<strong>Quality Grade - Better (AA): </strong> Top 25% of enhanced blue diamond in terms of quality. &nbsp;Greenish Blue in Color. &nbsp;This quality is comparable to that used by mall jewelers and chain stores.';
					break;
				case 'Enhanced Blue DiamondAAA':
					return '<strong>Quality Grade - Best (AAA): </strong> Top 10% of enhanced blue diamond in terms of quality. &nbsp;Teal Blue in Color. &nbsp;This quality is comparable to that used by leading independent/family jewelers.';
					break;
			}
	...
	module.updateProductDetails = function(productId){
		...
		if (productId){
			...
			if(settings.childProducts[productId].stones.length < $('.stone-detail-box-container').length){
				$('.stone-detail-box-container:gt('+(settings.childProducts[productId].stones.length-1)+')').remove();
			}
			else{
			...
			for(var iCounter=0; iCounter < settings.childProducts[productId].stones.length;  iCounter++){	// looping all stones
				if($('.stone-detail-box-container:eq('+iCounter+')').length == 0){
					module.addStoneDetails(iCounter+1, settings.childProducts[productId].stones[iCounter]);
				}
			...
			//var element=document.getElementById("displaydescription");
			//element.innerHTML= settings.childProducts[productId].description;
	...
	module.addStoneDetails = function(stoneIndex, stone){
		...
		$('.stones-details-box').append('<div class="stone-detail-box-container stone-detail-box'+oddOrEven+'">'+
			...
			'</div>'+
			'<div class="field">'+
				'<div class="fieldtitle">Approximate Dimensions:'+(((stone['name'] == 'Ruby' ||  stone['name'] == 'Blue Sapphire' || stone['name'] == 'Emerald' || stone['name'] == 'Tanzanite' || stone['name'] == 'Diamond' || stone['name'] == 'Enhanced Black Diamond' || stone['name'] == 'Enhanced Blue Diamond') && stone["shape"] != 'Trapezoid' && stone["shape"] != 'Half Moon' && stone["shape"] != 'Marquise' && stone["shape"] != 'Baguette')?'<span class="popup-icon" onclick="jewelProduct.showStonePopup(\'stone size\', \''+stone["name"]+'\', \''+stone["shape"]+'\')"></span>':'')+'</div>'+
				'<div class="fieldvalue dyn_stone'+stoneIndex+'_size"></div>'+
				'<div style="clear:both"></div>'+
			'</div>'+
			'<div class="field">'+
				'<div class="fieldtitle">Approximate Carat Total Weight:'+(((stone['name'] == 'Ruby' ||  stone['name'] == 'Blue Sapphire' || stone['name'] == 'Emerald' || stone['name'] == 'Tanzanite' || stone['name'] == 'Diamond' || stone['name'] == 'Enhanced Black Diamond' || stone['name'] == 'Enhanced Blue Diamond') && stone["shape"] != 'Trapezoid' && stone["shape"] != 'Half Moon' && stone["shape"] != 'Marquise' && stone["shape"] != 'Baguette')?'<span class="popup-icon" onclick="jewelProduct.showStonePopup(\'stone size\', \''+stone["name"]+'\', \''+stone["shape"]+'\')"></span>':'')+'</div>'+
				'<div class="fieldvalue dyn_stone'+stoneIndex+'_weight"></div>'+
				'<div style="clear:both"></div>'+
			'</div>'+
			'<div class="field">'+
				'<div class="fieldtitle">Quality Grade:'+(((stone['name'] == 'Ruby' ||  stone['name'] == 'Blue Sapphire' || stone['name'] == 'Emerald' || stone['name'] == 'Tanzanite' || stone['name'] == 'Diamond' || stone['name'] == 'Enhanced Black Diamond' || stone['name'] == 'Enhanced Blue Diamond') && stone["shape"] != 'Trapezoid' && stone["shape"] != 'Half Moon' && stone["shape"] != 'Marquise' && stone["shape"] != 'Baguette')?'<span class="popup-icon" onclick="jewelProduct.showStonePopup(\'stone quality\', \''+stone["name"]+'\', \''+stone["shape"]+'\')"></span>':'')+'</div>'+
