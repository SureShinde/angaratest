...
function adj_nav_show_products(transport)
{
	...
	jQuery("img").unveil();
	filterClose();			//	Close the filter panel
}
...
function adj_nav_apply_listener(evt)
{
	//alert('aply');
    /*var link = Event.findElement(evt, 'A'),
...
function adj_nav_init()
{
	...
	jQuery('.catalog-filter-nav-item:eq(0)').addClass('active');	//	S:VA
	jQuery('.catalog-filter-content:eq(0)').show();	
	...
	if(isXS){		
		...
		jQuery('.catalog-filter .catalog-filter-nav-item').on('vclick',function(event){
			if(!(isUpdating)){
				jQuery('.catalog-filter-content').hide();
				if(!(jQuery(this).hasClass('active'))){
					if(!(jQuery(this).parent().find('.catalog-filter-content').show()))
    ...
    var items, i, j, n, 
        classes = ['category', 'attribute', 'icon', 'price', 'clear', 'clearall', 'apply'];
