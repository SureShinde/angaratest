            ...
            onSuccess    : function(response) {
				...
				try{
					...
					if(res && typeof res.content != 'undefined' && document.getElementById('cart_help_callout')) { 
						...
						document.getElementById('cart-order-total').innerHTML = res.cartTotal;
					}
				} catch(e) {
					//console.log(e);
				}
...
/* Mobile specific chnages */
/* START */
jQuery(document).on('keyup','#estimate_postcode', function(e){
	if(e.keyCode == 13){
		e.preventDefault();
		estimateTaxRate(00000);
	}
	return false;
});
/* END */
...
function estimateTaxRate(old_estimate_postcode){
	...
	/* Mobile specific chnages */
	/* START */
	estimate_postcode = estimate_postcode.substr(0, 5);
	if(dataLoaded!='1' && estimate_postcode!=old_estimate_postcode){
	/* END */
		dataLoaded	=	1;
		jQuery('#buy-now-loader').removeClass('hide');
		jQuery('body').addClass('disabled');
