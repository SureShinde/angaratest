...
;(function($) {
	...
	$(function(){
		...
		$('.rating-links span, .review-count .rating-summary').click(function(){
			
			if($(window).width() < 768 && $(this).parents().hasClass('normal-pro-container')){
			 //alert(true);
			}
			...
			if($(window).width() < 768){
				$('#product-extra-info-tabs button[data-to-toggle=#customer-reviews-tab]').find('i.fa').addClass('fa-minus').removeClass('fa-plus');
			}
			$('#product-extra-info-tabs').find('.ress-nav-content').removeClass('show').addClass('hidden');
			$('#product-extra-info-tabs').find('#customer-reviews-tab').removeClass('hidden').addClass('show');
			if($(window).width() < 768){
	...
	/*$(function(){
		if(jQuery(window).width() > 767){
			//jQuery('#filter-panel').show();	//	S:VA	Commented to prevent the filter to show on load
	...
	$(function(){
		...
		$(document).on('click','*[data-toggle="tab"]', function(){
			if($($(this).data('to-toggle')).hasClass('hidden')){
				$('.' + $(this).data('to-toggle-class')).addClass('hidden');
				$($(this).data('to-toggle')).removeClass('hidden');
			...
			if($(window).width() < 768){
				if($(this).parents('.ress-nav').attr('id') == 'product-extra-info-tabs'){
	...
	/* Go To Top Button on Catalog Page. */
	/*$(function(){
	...
	/* Catalog Ajax Scroll */
	$(function(){
		if($('.category-product-list, .category-product-list-view, .product.item').length > 6){
	...
	$(function(){
		$('#special-offer-click').click(function(e){
			...
			if(typeof(productId) != 'undefined'){
				jQuery.get('/promotions/index/index/productId/'+productId+'/additionalCost/'+additionalCost, function(result){
...
function showNewsletterPopup(){
	//	S:VA
	if( jQuery(window).width() > 300 && (document.URL.indexOf('contacts/index/subscribe') == -1) && (document.URL.indexOf('checkout/cart') == -1) && (document.URL.indexOf('onepagecheckout/index/index') == -1) ){
...
var labels = jQuery('label');
    labels.each(function() {
        ...
        if (label.html() != '') {
             ...
             if(label.html()=='Trio1 Sku')
             {
               ...
               sku_trio1 = label.parent().next().find("input").val();
              // alert(sku_trio1);
             ...
             {
               sku_trio2 = label.parent().next().find("input").val();
             }
             if(label.html()=="Trio3 Sku")
             {
                sku_trio3 = label.parent().next().find("input").val();
             }
	...
	if(typeof(productId) != 'undefined'){
		jQuery('#view-offer-price, #price-with-discount').html('<i class="fa fa-spinner fa-spin apricot-text"></i>');
		if(typeof sku_trio1 !== 'undefined')
		{

			passedurl='/promotions/index/ajax/productId/'+productId+'/additionalCost/'+additionalCost+'/trio1/'+sku_trio1+'/trio2/'+
			sku_trio2+'/trio3/'+sku_trio3;
		}
		else
		{
			passedurl='/promotions/index/ajax/productId/'+productId+'/additionalCost/'+additionalCost;
		}
		if(typeof sku_trio1 !== 'undefined' && sku_trio1.length>0)
		{
			
			jQuery.get(passedurl, function(result){
				jQuery('#view-offer-price, #price-with-discount').html(result);
			})
		}
		else if(typeof sku_trio1 !== 'undefined' && sku_trio1.length==0)
		{ 
			if(secIteration)
			{
				setTimeout(function(){
...
function quickView(obj, callerId){
	...
	.append(
		jQuery('<div class="center-block quick-view-container" style="margin-top:100px; width:1005px;">').append(
			jQuery('<iframe class="no-border offscreen" frameborder="0" width="100%" src='+$this.attr('href')+'></iframe>').load(function(){
...
function navigation(){
	...
	jQuery('#navigation #main-menu').animate({left:'0px', opacity:'1'});
}
function navigationClose(){
	jQuery('#navigation #main-menu').animate({left:'-100%', opacity:'0'});
	jQuery('#navigation #main-menu ul.level1, #navigation #main-menu ul.level2').slideUp();
	jQuery('#navigation #main-menu a.level0, #navigation #main-menu a.level1, #navigation #main-menu a.level2').children('.fa').removeClass('fa-minus');
	jQuery('#navigation #main-menu a.level0, #navigation #main-menu a.level1, #navigation #main-menu a.level2').children('.fa').addClass('fa-plus');
...
function filterClose(){
	...
	jQuery('#products-grid, header, #catalogBannerMobile, .page-title, .catalog-slider').css('opacity','1');
	jQuery('#filter-panel').addClass('hidden');
}