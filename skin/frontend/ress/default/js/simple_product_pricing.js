    ...
    if(typeof(this.config.productImagesTrio) != 'undefined')
   {
    ...
    if(productId && this.config.productImages[productId].length > 0) {
        ...
        /* if(is_iPad){            
            ...
            var hasVideo = jQuery('#moreviews').find('ul li').hasClass('static-view');
            var customIndex = ((hasVideo && currentIndex > 1) ? currentIndex-1 : currentIndex);
            if(customIndex < 0){
            ...
            if($('image')){
                $('image').src = images[customIndex].url;
                   if(typeof(this.config.productImagesTrio) != 'undefined')
                {
                  //alert("hi");
            ...
            jQuery('#moreviews').find('ul li.static-view').each(function(i){
                if(i==0)
                    jQuery(this).insertAfter(jQuery('#moreviews').find('ul li:eq(2):not(.static-view)').last());
                else
  ...
  $.fn.Variations = function( config ) {
	...
	var module = this,
			...
			switch(attribute.code){
				case 'stone1_size':
					...
					if(typeof(settings.childProducts[option.products[0]]['total_weight']) != 'undefined'){
						...
						$('<div class="">').append(
							optionClickable.append('<span class="user-option-title carat-weight">'+optionLabel+'</span>')
					...
					if(typeof(settings.childProducts[option.products[0]]['weight_description']) != 'undefined')
						optionDetail.html('<strong>Approximate Carat Weight:</strong> '+settings.childProducts[option.products[0]]['weight_description']+'.');	
				...
				case 'stone1_grade':
					...
					optionContainer.append(
						$('<div class="">').append(optionClickable.append('<span class="user-option-title">'+label+'</span>'))
					);
					
					//RV: Add most popular
					/* if(label == 'Best'){
						optionContainer.append(
							$('<div class="padding-type-5 low-padding-left low-padding-right">').append(optionClickable.append('<span class="user-option-title">'+label+'</span><span class="most-popular-icon"><span>Most<br>Popular</span></span>'))
						);
					}else{
						optionContainer.append(
					...
					/* s:code added for the pearl project quality description */
					if(module.isPearlCategory(spConfig.getMatchingSimpleProduct())) {
						var stoneNameTemp = module.getSelectedProductStoneName(spConfig.getMatchingSimpleProduct());
						for(var j=0;j<stoneNameTemp.length;j++) {
							if(typeof stoneNameTemp[j] != 'undefined') {
								optionDetail.html(getStoneGradeDescription(stoneNameTemp[j], option.label, settings.stone1_shape));
							}
						}
					} else {
				...
				case 'metal1_type':
					...
					//RV: Add most popular
					/* if(option.label == '14K White Gold'){
						optionContainer.append(
							optionClickable.append('<span class="user-option-title">'+metalName+'</span><span class="most-popular-icon"><span>Most<br>Popular</span></span>')
						);
					}else{
						optionContainer.append(
							optionClickable.append('<span class="user-option-title">'+metalName+'</span>')
				...
				default:
					...
					var optionHtmlContent = $('<div class="padding-type-3">').append(optionClickable).append('<span class="user-option-title">'+option.label+'</span>').html();
		...
		optionClicked = function(option, attributeId, attribute, parent, optionContainer, optionDetailsContainer){
			parent.find('.default-user-option').removeClass('default-user-option').children().removeClass('active-option').addClass('no-transition');
			
			optionContainer.addClass('default-user-option').children().addClass('active-option no-transition');
				...
				$.each(settings.attributes,function(iCounter,attributeselected){
					...
					if(attributeselected.options.length > 1){
						...
						if(attributeselected.code == 'stone1_grade') {
							...
							for(var j=0;j<stoneNameTemp.length;j++) {
								if(typeof stoneNameTemp[j] != 'undefined') {
									optionHtmlSelected = getStoneGradeDescription(stoneNameTemp[j], selected.label, settings.stone1_shape);
		...
		optionMarkup = function(option, attributeId, attribute, parent, optionDetailsContainer, optionCount){
			if(optionCount >= 4){
				var opacity = 0;
			}else{
				var opacity = 1;
			}
			var optionContainer = $('<div style="opacity:'+opacity+'" class="option-container pull-left customize-product-option high-padding-left no-transition" id="option-container'+option.id+'">').appendTo(parent);
			var optionClickable = $('<div class="user-option-clickable product-dh relative-placeholder fix-border text-center clickable"></div>');
		...
		optionListMarkup = function(attributeId, attribute, parent){
			if(attribute.code!='ring_size'){
				var optionListContainer = $('<div class="option-list-container customize-product-tab-content relative-placeholder max-low-margin-bottom clearfix" id="option-list-container'+attributeId+'">');
				...
				$.each(attribute.options,function(iCounter,option){
					optionMarkup(option, attributeId, attribute, optionListClickableOptions, optionDetailsContainer, optionCount)
					optionCount++;
				});
				
				// Commented by :RV - to add title for option
				optionListContainer.append('<div class="customize-btn-title pull-left"><span class="select">Select</span> <span class="title">'+(attribute.code == 'stone1_size'?'Carat Weight':(attribute.code == 'stone1_grade'?'Gemstone Quality':attribute.label))+'</span></div>');
				
				if(attribute.code == 'stone1_grade'){
					optionListContainer.append($('<div class="option-hldr pull-left">').append(optionListClickableOptions)).prependTo(parent);
					optionListContainer.append($('<div class="cs-option-details">').append($('<div class="option-details-container hidden">').append(optionDetailsContainer)));
				}else{
					optionListContainer.append($('<div class="option-hldr pull-left">').append(optionListClickableOptions)).appendTo(parent);	
					optionListContainer.append($('<div class="cs-option-details">').append($('<div class="option-details-container hidden">').append(optionDetailsContainer)));
				}
			}
		},
		...
		variationsMarkup = function(){
			...
			var optionsContainer = $('<div class="customize-panel-container clearfix">');
		...
		selectTrioBox = function(width, height){
            ...
            $('.trio-box').css({'position':'absolute','width':elemWidth+'px','top':topPosition+'px', 'left':leftPosition+'px','z-index':'99'}).show();
        },
		...
		/*
			This will rendor the super attribute description
		*/
	...
	// Public functions		
	//	S:VA
	module.getOfferPrice = function(productId){
		var childProducts 	= 	spConfig.config.childProducts;
		var offerPrice 		= 	childProducts[productId]["finalPrice"];
		var productVariationPrice 		= 	offerPrice;		//	S:VA
		var couponPercent	=	spConfig.config.coupon_percent;
		var additionalCost = 0;
		jQuery.each(optionsPrice.customPrices,function(index1, customPriceObj){
			if(customPriceObj.priceValue)
				additionalCost += parseFloat(customPriceObj.price)
		});
		if(additionalCost>0){
			var	totalPrice		=	parseFloat(offerPrice) + additionalCost;
			offerPrice 			= 	totalPrice - (totalPrice * (couponPercent / 100));
		}else{
			offerPrice 			= 	offerPrice - (offerPrice * (couponPercent / 100));
		}
		offerPrice=Math.round(offerPrice);
		if (productId && offerPrice) {
			$$('#product_addtocart_form span#view-offer-price').each(function(el) {
				el.innerHTML = optionsPrice.formatPrice(offerPrice);
			});
		}
	};	
	//	E:VA
	...
	module.getSelectedProductStoneName = function(productId){
		...
		if(module.isPearlCategory(productId) && typeof(settings.childProducts[productId].stones[0]['name']) != 'undefined'){
			stone1Name.push(settings.childProducts[productId].stones[0]['name']);
			var products = Object.keys(settings.childProducts);
			for(var i=0;i<products.length;i++) {
				if(typeof products[i] != 'undefined' && typeof(settings.childProducts[products[i]].stones[0]['name']) != 'undefined') {
					if(stone1Name.indexOf(settings.childProducts[products[i]].stones[0]['name']) < 0) {
						stone1Name.push(settings.childProducts[products[i]].stones[0]['name']);
					}
				}
			}
		}	
		else{
			stone1Name.push(settings.stone1_name);	
		}		
		return stone1Name;		
	};
	...
	module.updateProductDetails = function(productId){
		...
		if (productId){			
			...
			specialOfferPrice();		//	calling offer price change on click of variation change
