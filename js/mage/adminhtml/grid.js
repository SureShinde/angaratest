...
function colorize () {
	try{
		var i = 0;
		var j = 0;
		tbl = document.getElementById('sales_order_grid_table');
		var index=-1;
		for(i=0;i<tbl.rows[0].cells.length;i++)
		{
			str = tbl.rows[0].cells[i].childNodes[0].innerHTML;
			if(str.indexOf('OK TO SHIP') > -1)
			{
				index = i;
				//tbl.rows[0].cells[i].style.display = 'none';
				//tbl.rows[1].cells[i].style.display = 'none';
				
			}
		}
		for(i=0;i<tbl.childNodes.length;i++)
		{
			if(tbl.childNodes[i].tagName == 'COLGROUP')
			{
				for(j=0;j <tbl.childNodes[i].childNodes.length;j++)
				{
					if(tbl.childNodes[i].childNodes[j].width=="1")
					{
						tbl.childNodes[i].childNodes[j].style.display = 'none';
					}
				}
			}
		}
		if(index != -1)
		{
			...
			{
				if(tbl.rows[j].cells[index].childNodes[0].data.replace(/^\s+|\s+$/gm,'')=='Cancel')
				{
					tbl.rows[j].style.backgroundColor = "white";
				}
				if(tbl.rows[j].cells[index].childNodes[0].data.replace(/^\s+|\s+$/gm,'')=='Yes')
				{
					tbl.rows[j].style.backgroundColor = "lightblue";
				}
				if(tbl.rows[j].cells[index].childNodes[0].data.replace(/^\s+|\s+$/gm,'')=='No')
				{
					tbl.rows[j].style.backgroundColor = "pink";
				}
				//tbl.rows[j].cells[index].style.display = 'none';
			}
		}
	}
	catch(ex)
