    ...
    <div class="row">
        <form role="form" method="post" enctype="multipart/form-data" name="frm_product_tags" action="">
          <div class="form-group">
            <label for="input_file">Please upload a <a href="remove_related_products.csv" title="Download sample file">csv file</a> with sample sku to remove upsell products :</label>
...
function updateData( $data, $resource ) {
	...
	if($productId){
		try {
			...
			$transaction->query("DELETE FROM ". $table ." WHERE  product_id = '".$productId."' AND link_type_id = '4'");
			$transaction->commit();
